...
We can plot the different layers inside the raster brick.

Plot the top layer
```{r}
plot(rasbr[[1]]) #the double brackets tell R which layer (which corresponds to my depth) to plot
```

plot layer 20
```{r}
plot(rasbr[[20]]) #the double brackets tell R which layer (which corresponds to my depth) to plot
```
...
```{r}
obs <- read.csv("../data/bio/observations.csv") #change this path to your .csv location
head(obs)
```
