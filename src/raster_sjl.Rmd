...
for (i in 1:no_netcdf) {
  ...
  brkyr <- sapply(strsplit(netcdf_name[i], "_"), "[[", 1) # here I am extracting the first part of the filename deliminated with _ (which is the year)
  brkmth <- sapply(strsplit(netcdf_name[i], "_"), "[[", 2) # here I am extracting the second part of the filename deliminated with _ (which is the month)
  ...
  temp_brick <- projectRaster(temp_brick, aea) #aea is an existing raster in the projection I want that I created (environmental_data_preperation)
    for (j in 1:length(obssp)) {  # CYCLES THROUGH THE ROWS
      yr <- obssp$year[j]
      mth <- obssp$month[j]
      if (brkyr == yr &  brkmth == mth){
        de <- obssp$depth[[j]]  #have a variable for the depth layer
        obssp$samtest[j] <- extract(x=temp_brick[[de]], y = obssp) #this is the equation you will be calculating for every value of y
        obssp$samtesttop[j] <- extract(x=temp_brick[[1]], y = obssp) #this is the equation you will be calculating for every value of y
        print(j)
      }
    }
}
