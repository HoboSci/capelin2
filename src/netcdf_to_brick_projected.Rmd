...
```
just a quick test....
(also not sure why the names are wrapped in '')
```{r}
`1998_01_salinity.nc`
...
```{r}
library(sp)
xy <- data_aea[ , c("decimalLongitude", "decimalLatitude")]
...
I want to populate the following fields in the species_obs_date spatialdf
mlp
salinty_1
salinity_depth
ssh
...
where _1 indicates the surface layer [[1]] of each environmental variable, and depth is the == species_obs_date$depthlayerno
