...
backna2
write.csv(background_all, "../output/bio/background_point_na_removal_nona", row.names = FALSE)
head(testbackground_all)
```

```{r}
testbackground_all <- subset(testbackground_all, select = -c(temp_depth, temp_surface, salinity_depth, salinity_surface, o2_depth, o2_surface, chl_depth, chl_surface, ssh_surface, mlp_surface))
head(testbackground_all)
```
```{r}
testbackground_all <- subset(testbackground_all, select = -c(longitude_.1, latitude_m.1, optional))
head(testbackground_all)
```
