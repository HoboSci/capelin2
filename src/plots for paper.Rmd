...
packages <- c("stringr", "reshape2", "ggplot2", "Hmisc", "viridis", "raster", "RColorBrewer")
source("../src/ipak.R")
ipak(packages)
```
...
for (m in 1:length(mthord)){
  ...
  mth <- mthord[m]
  bin <- binlst[b]
  ascdepth <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, "_binned_", bin, ".*asc"), full.names = TRUE)
...
ggplot(mar, aes(Depth, p_cells, fill = Probability)) + geom_col() + coord_flip() +  scale_fill_manual(values = probcol) + scale_x_discrete(limits = flevelsm, breaks=flevelsm[seq(1,length(flevelsm), by = 2)]) + coord_flip()

ggplot(jun, aes(Depth, p_cells, fill = Probability)) + geom_col() + coord_flip() +  scale_fill_manual(values = probcol) + scale_x_discrete(limits = flevelsj, breaks=flevelsj[seq(1,length(flevelsj), by = 2)]) + coord_flip()

ggplot(sep, aes(Depth, p_cells, fill = Probability)) + geom_col() + coord_flip() +  scale_fill_manual(values = probcol) + scale_x_discrete(limits = flevelss, breaks=flevelss[seq(1,length(flevelss), by = 2)]) + coord_flip()

ggplot(dec, aes(Depth, p_cells, fill = Probability)) + geom_col() + coord_flip() +  scale_fill_manual(values = probcol) + scale_x_discrete(limits = flevelsd, breaks=flevelsd[seq(1,length(flevelsd), by = 2)]) + coord_flip()
```
