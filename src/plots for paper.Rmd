...
clogasc_list <- list.files(paste0("../output/maxent/predictions/nov"), pattern = "^avg_cloglog_.*.asc", full.names = TRUE) #just choose a random month to calculate cells
...
write.csv(cumst1, "../output/maxent/paper_plots/prob_by_depth/cellsperlayer.csv", row.names = FALSE)
...
for (m in 1:length(mthord)){
  ...
  mth <- mthord[m]
  bin <- binlst[b]
  ascdepth <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, "_binned_", bin, "_.*asc"), full.names = TRUE)
  ...
  for (lyr in 1:nlayers(binaryavgstack)){
    ascname <-  str_sub(ascdepth[lyr], 63, -5)
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  probblst <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, ".*probbin_.*.csv"), full.names = TRUE)
...
for (m in 1:length(mthord)){
  ...
  for (cum in 1:length(clogasc_list)){
    cumasc <- raster(clogasc_list[cum])
    ascname <- names(cumasc)
    cumasc <- raster::mask(x = cumasc, mask = outsidemcp, inverse = TRUE)
    probbin <- calc(cumasc, fun = binprob) #creates raster layer with cell values based on the binprob function
    writeRaster(probbin, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_binned_avgprob_", ascname, ".asc"), overwrite = TRUE)
    #plot(probbin, png(paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_binned_avgprob_", ascname, ".png")), overwrite = TRUE)
    #dev.off() # stops automatic saving of the plot to a png
  }
}
```
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  ascdepth <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, "_u0.4_binned_avgprob_", ".*asc"), full.names = TRUE)
  ...
  write.csv(stackstathigh, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_probbin_cellcount_bydepth_avg.csv"), row.names = FALSE)
}
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  probblst <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, "_u0.4_probbin_cellcount_bydepth_avg.csv"), full.names = TRUE)
  stackstat <- read.csv(probblst[1])

  
  colnames(stackstat) <- c("depth", "0 - 0.39")
  stackstat <- melt(stackstat, id = "depth")
  stackstat <- stackstat[!(stackstat$depth == "1151.99"), ]
  stackstat <- stackstat[!(stackstat$depth == "1265.86"), ]
  stackstat <- stackstat[!(stackstat$depth == "1387.38"), ]
  ...
  write.csv(stackstat, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_allprobbin_cellcount_bydepth_avg.csv"), row.names = FALSE)
}
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  a <- read.csv(paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_allprobbin_cellcount_bydepth_avg.csv"), header = TRUE, stringsAsFactors=FALSE)
  for (r in 1:nrow(a)){
    a$Depth[r] <- gsub("prob_avg_cloglog_", "", x = a$Depth[r])
    d1 <- str_sub(a$Depth[r], end = 4)
	  for (ro in 1:nrow(cumst1)){
      d2 <- str_sub(cumst1$depth[ro], end = 4)
      if (d1 == d2){
  ...
  write.csv(a, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_dis_prob_cells.csv"), row.names = FALSE)
}
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  a <- read.csv(paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_u0.4_dis_prob_cells.csv"), header = TRUE, stringsAsFactors=FALSE)
  ...
  write.csv(c, paste0("../output//maxent/paper_plots/prob_by_depth/", mth, "_u0.4plus_dis_prob_cells.csv"))
}
```
