...
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps", "lattice", "tidyr", "enmSdm", "ggplot2", "plyr", "reshape2", "tidyverse", "patchwork", "purrr", "Hmisc")
source("../src/ipak.R")
ipak(packages)
```
...
for (f in 1:length(alst)){
  ...
  for (cum in 1:length(clogasc_list)){
    ...
    writeRaster(probbin, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_", ascname, ".asc"), overwrite = TRUE)
    plot(probbin, png(paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_", ascname, ".png")), overwrite = TRUE)
    dev.off() # stops automatic saving of the plot to a png
  }
}
...
ggsave("../output/maxent/paper_plots/prob_by_depth.png", width = 210, height = 297, units = "mm", dpi = 600)
```

# sample probabily maps 
...
#2d maps - max kappa thresholds

this creates binary maps for each layer based on the maxkappa threshold
```{r}
binprob <- function(x){
  ifelse(x > 0 & x >= mk, 1,
  ifelse(x <= 0, 0, NA))
...
for (r in 1:nrow(sum2d)){
  sum2d$proportion[r] <- (sum2d$count[r]/totc)*100 #this actually shows percentage
}
...
kapmmd$range <- kapmmd$max - kapmmd$min #get depth range
write.csv(kapmmd, "../output/maxent/paper_plots/hotspot_mapping/kappa-min-max-depth.csv")
```

#get lat long position of most northely, southerly, western, and eastern cell centroid (max kap)

```{r maxkap cell centroid}
