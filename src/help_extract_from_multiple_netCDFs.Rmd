...
An example of the `netCDF` is available [here](https://drive.google.com/file/d/1y2XU5RzUBRLgTXkql667a5o_PVk5LMTP/view?usp=sharing) . To use the netCDFs in R, I have made a loop to create `raster bricks` and project them in an equal-area grid:
...
Note that this may not be the most computationally efficient way to do this as I will be creating a lot of raster bricks that will be stored to memory… 
...
```{r}
library(sp)
xy <- occurrence[ , c("decimalLongitude", "decimalLatitude")]
species_obs_date <- SpatialPointsDataFrame(coords = xy, data = occurrence, proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")) #convert into a spatialpoints dataframe to do the extraction
