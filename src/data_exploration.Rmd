...
asclist <- (list.files("../data/jan_test_86.9", pattern = glob2rx("*1998_01*_86.9294*.asc"), full.names = TRUE))    # this folder currently contains other dummy files. its just a test to automate stacks
ascfile <- names(asclist[[1]])
ascyear <- as.character(sapply(strsplit(ascfile[[1]], "_"), "[[", 1))
ascmonth <- as.character(sapply(strsplit(ascfile[[1]], "_"), "[[", 2))
ascdepth <- as.character(sapply(strsplit(ascfile[[1]], "_"), "[[", 5))
ascname <- paste(ascyear, ascmonth, ascdepth, sep = "_") #once you figure out how to rename the layers in the stack, you will rename the stack this () or not - maybe just manual
asclist # now need to rename to match maxent expectations
...
library(dismo)
library(rJava)
jar <- paste(system.file(package="dismo"), "C:/Users/bunny/Documents/GitHub/Chapter_1/packrat/lib/x86_64-w64-mingw32/3.5.1/dismo/java/maxent.jar", sep='')
file.exists(jar)
testmax <- maxent(janpresabmax, janpresabmax$occurrence)
