...
write.csv(vif_allo2_depth_sq, "../output/bio/vif_allo2_depth_sq", row.names = TRUE)
vif_allo2_depth_sq
```

without gear
```{r}
vif_allbuttemp_surfacegear <- vif(lm(occurrence ~ temp_depth + salinity_surface + salinity_depth + o2_surface + o2_depth + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + gear + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
write.csv(vif_allbuttemp_surfacegear, "../output/bio/vif_allbuttemp_surfacegear.csv", row.names = TRUE)
vif_allbuttemp_surfacegear

vif_allo2_depthgear <- vif(lm(occurrence ~ temp_surface + temp_depth + salinity_surface + salinity_depth + o2_surface + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + gear + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
