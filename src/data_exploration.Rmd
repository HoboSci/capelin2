...
so ther is a question between temp-surface and o2_depth (-0.51923)... let's look at this

with gear
```{r}
vif_allbuttemp_surface <- vif(lm(occurrence ~ temp_depth + salinity_surface + salinity_depth + o2_surface + o2_depth + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + gear + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
write.csv(vif_allbuttemp_surface, "../output/bio/vif_allbuttemp_surface.csv", row.names = TRUE)
vif_allbuttemp_surface

vif_allbuttemp_surface_sq <- read.csv("../output/bio/vif_allbuttemp_surface.csv", header = TRUE)
vif_allbuttemp_surface_sq$GVIF2Dfsq <- vif_allbuttemp_surface_sq$GVIF..1..2.Df..^2
write.csv(vif_allbuttemp_surface_sq, "../output/bio/vif_allbuttemp_surface_sq.csv", row.names = TRUE)
vif_allbuttemp_surface_sq

vif_allo2_depth <- vif(lm(occurrence ~ temp_surface + temp_depth + salinity_surface + salinity_depth + o2_surface + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + gear + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
write.csv(vif_allo2_depth, "../output/bio/vif_allo2_depth.csv", row.names = TRUE)
vif_allo2_depth

vif_allo2_depth_sq <- read.csv("../output/bio/vif_allo2_depth.csv", header = TRUE)
vif_allo2_depth_sq$GVIF2Dfsq <- vif_allo2_depth_sq$GVIF..1..2.Df..^2
write.csv(vif_allo2_depth_sq, "../output/bio/vif_allo2_depth_sq.csv", row.names = TRUE)
vif_allo2_depth_sq
```

without gear
```{r}
vif_allbuttemp_surfacegear <- vif(lm(occurrence ~ temp_depth + salinity_surface + salinity_depth + o2_surface + o2_depth + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
write.csv(vif_allbuttemp_surfacegear, "../output/bio/vif_allbuttemp_surfacegear.csv", row.names = TRUE)
vif_allbuttemp_surfacegear

vif_allo2_depthgear <- vif(lm(occurrence ~ temp_surface + temp_depth + salinity_surface + salinity_depth + o2_surface + chl_surface + chl_depth + bottom_depth + mlp_surface + ssh_surface + nao_sample + nao_prev + nao_winter + amo_sample + amo_prev + amo_winter, data = presab))
write.csv(vif_allo2_depthgear, "../output/bio/vif_allo2_depthgear.csv", row.names = TRUE)
vif_allo2_depthgear
```


```{r}
vif_test <- vif(lm(occurrence ~ temp_surface + temp_depth + salinity_surface + salinity_depth + chl_surface + bottom_depth + mlp_surface + nao_sample + nao_prev + nao_winter + amo_sample + amo_winter, data = presab))
vif_test
```
...
the data is in presab$total_cell_obs_xyt 
```{r}
unique_pointpercell <- unique(presab$total_cell_obs_xyzt)
unique_pointpercell
```
bugger... ok so lets do a frequency table
```{r}
celobsmore1 <- subset(presab, total_cell_obs_xyzt > 1)
pointpercell <- with(celobsmore1, table(cell_id_xyzt, total_cell_obs_xyzt))
write.csv(pointpercell, file = "../output/bio/pointpercell.csv")
nrow(pointpercell)
pointpercell
```

so there are 949 3d timesliced cells that contain more than one occurrence. 
