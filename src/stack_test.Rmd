...
try changing some of the settings here - particularly you want aic
```{r}
...
from dismo thing... but with my bad code... seems to need to have 
```{r}
...
occtrain <- train2003
occtrainpa <- occtrain$occurrence
occtrain <- subset(occtrain, select = -c(year, occurrence))

me2 <- maxent(x = occtrain, p = occtrainpa, file = .asc, args=c("maximumiterations=5000", "responsecurves=TRUE", "jackknife=TRUE", "threads=8", "addsamplestobackground=TRUE"))
me2

occtest <- test2003
occtrain <- train2003
e1 <- evaluate(p= occtrain, a= occtest, me2) #@models[[2]]
e1

pvtest <- data.frame(extract(predictors, occtest))
avtest <- data.frame(extract(predictors, bg))
e2 <- evaluate(me2, p=pvtest, a=avtest)
e2
...
# trim layer by polygon

C:\Users\bunny\Documents\GitHub\Chapter_1\output\maxent\predictions\jun\avg_cloglog_147.406.asc
C:\Users\bunny\Documents\GitHub\Chapter_1\output\bio\minimum_convex_poly\mcp_capelin_100_poly.shp
C:\Users\bunny\Documents\GitHub\Chapter_1\output\env\aea.tif

```{r}
testras <- raster("../output/maxent/predictions/jun/avg_cloglog_147.406.asc")
mcp <- readOGR("../output/bio/minimum_convex_poly/mcp_capelin_100_poly.shp") 
aea <- raster("../output/env/aea.tif")
plot(testras)
plot(mcp)
class(mcp)
rr <- mask(testras, mcp)
plot(rr); plot(mcp, add = TRUE)
plot(testras); plot(mcp, add = TRUE)
plot(rr);plot(testras)
```
...
plot(us)
class(us)
projection(testras) <- CRS(aea)

us <- spTransform(us, CRS(projection(aea)))
...
a <- plot(testras); plot(us, add = TRUE)
