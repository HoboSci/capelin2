...
mar11 <- raster("../output/maxent/predictions/com-split/mar/avg_cloglog_11.7737.asc")
mar61 <- raster("../output/maxent/predictions/com-split/mar/avg_cloglog_61.1128.asc")
mar199 <- raster("../output/maxent/predictions/com-split/mar/avg_cloglog_199.79.asc")
mar628 <- raster("../output/maxent/predictions/com-split/mar/avg_cloglog_628.026.asc")
mar11@crs <- acrs
mar61@crs <- acrs
mar199@crs <- acrs
mar628@crs <- acrs
...
jun11 <- raster("../output/maxent/predictions/com-split/jun/avg_cloglog_11.7737.asc")
jun61 <- raster("../output/maxent/predictions/com-split/jun/avg_cloglog_61.1128.asc")
jun199 <- raster("../output/maxent/predictions/com-split/jun/avg_cloglog_199.79.asc")
jun628 <- raster("../output/maxent/predictions/com-split/jun/avg_cloglog_628.026.asc")
jun11@crs <- acrs
jun61@crs <- acrs
jun199@crs <- acrs
jun628@crs <- acrs
...
sep11 <- raster("../output/maxent/predictions/com-split/sep/avg_cloglog_11.7737.asc")
sep61 <- raster("../output/maxent/predictions/com-split/sep/avg_cloglog_61.1128.asc")
sep199 <- raster("../output/maxent/predictions/com-split/sep/avg_cloglog_199.79.asc")
sep628 <- raster("../output/maxent/predictions/com-split//sep/avg_cloglog_628.026.asc")
sep11@crs <- acrs
sep61@crs <- acrs
sep199@crs <- acrs
sep628@crs <- acrs
...
dec11 <- raster("../output/maxent/predictions/com-split/dec/avg_cloglog_11.7737.asc")
dec61 <- raster("../output/maxent/predictions/com-split/dec/avg_cloglog_61.1128.asc")
dec199 <- raster("../output/maxent/predictions/com-split/dec/avg_cloglog_199.79.asc")
dec628 <- raster("../output/maxent/predictions/com-split/dec/avg_cloglog_628.026.asc")
dec11@crs <- acrs
dec61@crs <- acrs
dec199@crs <- acrs
dec628@crs <- acrs
...
#2d maps for comb-maxent

#2d hotspot maps

first create layers that are 0.5 and above

```{r habitat classification on probability - >0.5xx}
...
for (m in 1:length(mthord)){
  mthna <- mthord[m]
  clogasc_list <- list.files(paste0("../output/maxent/predictions/com-split/", mthna), pattern = "^avg_cloglog_.*asc", full.names = TRUE)
  for (cum in 1:length(clogasc_list)){
    cumasc <- raster(clogasc_list[cum])
    ascname <- names(cumasc)
    ascname <- gsub("avg_cloglog_", "", ascname)
...
ss <- shapefile("../data/env/landp/land-wgs84v2.shp") #made in arcgis
crs(ss) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0 "
acrs <- crs(ss)
mcp <- shapefile("../output/bio/minimum_convex_poly/mcp-wgs84.shp")
#ss <- raster::intersect(s, mcp)  #trim to mcp
...
s <- st_read("../data/env/landp/land-wgs84v2.shp") %>% 
  st_set_crs(proj)

mcp <- st_read("../output/bio/minimum_convex_poly/mcp-wgs84.shp")

filelist <- list.files("../data/plot2-probmaps/",pattern="avg_cloglog*",recursive = TRUE,include.dirs = TRUE)

x <- stack(file.path("../data/plot2-probmaps/",filelist)) %>% 
  mask(mcp) 
...
df <- x %>% 
  ...
  mutate(variable=gsub(".asc","",variable),
         month=factor(months(parse_date_time(gsub("\\..*$", "",variable),"%m")),
                      ordered = TRUE,
                      levels=months(parse_date_time(1:12,"%m"))),
         depth=as.numeric(gsub("^.*_", "",variable)))

#remi code stops here

#here i capitalise the months
df$month <- str_sub(df$variable, start = 16, end = 18)
...
df$month <- sapply(df$month, CapStr)

#remi code starts here

depthlab <- paste(unique(df$depth),"m")[c(1,3,2,4)]
names(depthlab) <- unique(df$depth)[c(1,3,2,4)]

ggplot(df)+
  geom_sf(data=mcp,fill='grey')+
  geom_raster(aes(x,y,fill=value))+
  geom_sf(data=mcp,fill="transparent")+
  scale_fill_viridis(option="magma",
                       direction=-1, limits = c(0,1), breaks = c(0, 0.25, 0.5, 0.75, 1),
                       guide=guide_colourbar(barwidth=0.5, barheight = 15, title = NULL, draw.ulim = FALSE, draw.llim = FALSE))+
  facet_grid(depth~month,
             labeller = labeller(depth=depthlab))+
  theme_classic()+
...
ggsave("../output/maxent/paper_plots/test-overview.png", width = 210, height = 297, units = "mm", dpi = 600)
```
...
mcp <- st_read("../data/plot4-variables/minimum_convex_poly/mcp-wgs84.shp")

s <- st_read("../data/plot4-variables/landp/land-wgs84v2.shp") %>% 
  st_set_crs(proj) %>% 
  st_intersection(mcp)

filelist <- list.files("../data/plot4-variables/ascii/ascii1998/",pattern="*.asc",recursive = TRUE)

x <- stack(file.path("../data/plot4-variables/ascii/ascii1998/",filelist)) %>% 
  mask(mcp) %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
  gather(key = "variable", value = "value",-x,-y) %>% 
  separate(variable,c('year','month','variable','depth'),'_',remove=TRUE) %>% 
  mutate(month=factor(months(parse_date_time(month,"%m")),
                      ordered = TRUE,
                      levels=months(parse_date_time(1:12,"%m"))),
         year=as.numeric(gsub("X","",year)),
         monthyear=factor(paste0(month,'\n',year),
                          ordered = TRUE,
                          levels=unique(paste0(month,'\n',year))),
         variable=gsub(".nc","",variable)
         )


vars = c('chl','o2','salinity','temp')
...
for(v in vars){
  if(v=='chl'){
   assign(v,ggplot(x %>% filter(variable==v,month!='September'))+
           geom_sf(data=s,fill='grey')+
           geom_raster(aes(x,y,fill=value))+
           geom_sf(data=mcp,fill="transparent")+
            scale_fill_viridis(option="viridis", direction = -1, limits = c(0.03, 4.5), 
             #scale_fill_distiller(palette="Spectral", direction=-1)+
           guide_colourbar(barwidth=0.5, title = "mmol\u207B\u00B3", draw.ulim = FALSE, draw.llim = FALSE))+
           labs(title= "Chl \u03b1")+ #i added
           facet_wrap(~monthyear,nrow=4,strip.position = "right")+
           theme_classic()+
           scale_x_continuous(breaks = seq(-70,-50,10))+
      ...
      assign(v,ggplot(x %>% filter(variable==v,month!='September'))+
           ...
           geom_raster(aes(x,y,fill=value))+
           geom_sf(data=mcp,fill="transparent")+
           scale_fill_viridis(option="viridis", direction = -1, limits = c(207, 418) , 
           #scale_fill_distiller(palette="Spectral", direction=-1)+
           guide_colourbar(barwidth=0.5, title = "mmol\u207B\u00B3", draw.ulim = FALSE, draw.llim = FALSE))+
           labs(title=paste0("Dissolved ", sapply(v, CapStr)))+ #i added
      ...
      assign(v,ggplot(x %>% filter(variable==v,month!='September'))+
           ...
           geom_raster(aes(x,y,fill=value))+
           geom_sf(data=mcp,fill="transparent")+
           scale_fill_viridis(option="viridis", direction = -1, limits = c(22, 36), 
           #scale_fill_distiller(palette="Spectral", direction=-1)+
           guide_colourbar(barwidth=0.5, title = "PSU", draw.ulim = FALSE, draw.llim = FALSE))+

           facet_wrap(~monthyear,nrow=4,strip.position = "right")+
           labs(title=sapply(v, CapStr))+ #i added
           theme_classic()+
           scale_x_continuous(breaks = seq(-70,-50,10))+
           theme(legend.position = "bottom",
                 axis.title.x=element_blank(),
    ...
    assign(v,ggplot(x %>% filter(variable==v,month!='September'))+
             ...
             geom_raster(aes(x,y,fill=value))+
             geom_sf(data=mcp,fill="transparent")+  
             scale_fill_manual(col=blue2red(20), 
             #scale_fill_viridis(option="viridis", direction = -1, limits = c(270, 296),
             #scale_fill_distiller(palette="Spectral", direction=-1)+
             guide_colourbar(barwidth=0.5, title = "Kelvin", draw.ulim = FALSE, draw.llim = FALSE))+
             facet_wrap(~monthyear,nrow=4,strip.position = "right")+
             labs(title=sapply(v, CapStr))+ #i added
             theme_classic()+
             scale_x_continuous(breaks = seq(-70,-50,10))+
...
chl|o2|salinity|temp

#ggsave("../output/maxent/paper_plots/variable_samples.png", height = 210, width = 297, units = "mm", dpi = 600)
```
...
s <- st_read("../data/plot3-2d/landp/land-wgs84v2.shp") %>% 
  st_set_crs(proj) %>% 
  st_intersection(mcp)

filelist <- list.files("../data/plot3-2d/hotspot-mapping/",include.dirs = TRUE)

x <- stack(file.path("../data/plot3-2d/hotspot-mapping/",filelist)) %>% 
  mask(mcp) %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
  gather(key = "variable", value = "value",-x,-y) %>% 
  mutate(month=factor(months(parse_date_time(gsub("_.*$", "",variable),"%m")),
                      ordered = TRUE,
                      levels=months(parse_date_time(1:12,"%m"))),
         value=ifelse(value==1,">0.5","<0.5")) 

df <- x %>% 
  group_by(month) %>% 
  summarize(total=round(sum(value==">0.5")/n(),2))

ggplot(x)+
  geom_sf(data=s,fill='grey',colour='transparent')+
  geom_raster(aes(x,y,fill=value))+
  scale_fill_manual(values=c('transparent','red'))+
  geom_sf(data=mcp,fill="transparent")+
  geom_text(data=df,x=-50,y=67,label=df$total)+
...
ggsave("../output/maxent/paper_plots/two-d.png", height = 210, width = 297, units = "mm", dpi = 600)
...
#2-d map edited for value as remi cunted of the cells in the layer i sent and not with all the layers.
...
binaryprob_list <- list.files("../output/maxent/paper_plots/hotspot_mapping", pattern = ".*_bin_0.5_2d.asc", full.names = TRUE)
sum2d <- data.frame(matrix(ncol = 2, nrow = 10))
sum2d <- setNames(sum2d, c("month", "count"))
...
#get proportion
tcs <- read.csv("../output/maxent/paper_plots/cellsperlayer.csv", header = TRUE) #file showing how many cells per depth
#tcs <- tcs[-c(1:3), ] #because the top three rows are deeper than the capelin obs...
totc <- sum(tcs$no_cells)
...
#quickest way might be just to inject my csv file with the correct values here....
df2 <- read.csv("../output/maxent/paper_plots/cell_count_0.5.csv", header = TRUE)
df$total <- df2$proportion
df$total <- format(round(df$total, digits = 2))
#colnames(df) <- c("month", "cellc", "total")
...
#binned probability cell count for the whole 3d space per month

```{r}
library(stringr)

#calculate no of cells in the 3D space
prob <- read.csv("../output/maxent/paper_plots/prob_by_depth/apr_dis_prob_cells.csv", header = TRUE) #just a randomly chosen dataset
t2 <- subset(prob, Probability == "0 - 0.09") #could have chosen any probability
tc <- sum(t2$total_cells) #173623

dislst <- list.files("../output/maxent/paper_plots/prob_by_depth/", pattern = "dis_prob_cells.csv", full.names = TRUE)
totlst <- list()
for (m in 1:length(dislst)){
  pr <- read.csv(dislst[m])
  mth <- str_sub(dislst[m], start = 44, end = 46)
  pr2 <- aggregate(pr$Cells, by=list(Category = pr$Probability), FUN = sum)
  pr2$percent <- (pr2$x/tc)*100
  pr2$month <- mth
  write.csv(pr2, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_cellprob_totals.csv"), row.names = FALSE)
  totlst[[m]] <- pr2
}

#then i just made a table manually in excel
```

#max kappa thresholds

this creates binary maps based on th#max kappa thresholds

this creates binary maps based on the maxkappa threshold
```{r}
binprob <- function(x){
  ifelse(x > 0 & x >= mk, 1,
  ifelse(x <= 0, 0, NA))
...
#get the maxkap value
alst <- list.files("../output/maxent/models/test-comb/", pattern = "avgAIC.csv", full.names = TRUE)

for (f in 1:length(alst)){
  mth <- str_sub(alst[f], start = 35, end = 37)
  af <- read.csv(alst[f], header = TRUE)
  af <- tail(af, n = 1)
  mk <- af$test_maxkap

#create binary maps per depth layer
  clogasc_list <- list.files(paste0("../output/maxent/predictions/com-split/", mth), pattern = "^avg_cloglog.*.asc", full.names = TRUE)
  for (cum in 1:length(clogasc_list)){
    cumasc <- raster(clogasc_list[cum])
    ascname <- names(cumasc)
    probbin <- calc(cumasc, fun = binprob) #creates raster layer with cell values based on the binprob function
    writeRaster(probbin, paste0("../output/maxent/hotspot_mapping/prob_by_depth/", mth, "_maxkap_", ascname, ".asc"), overwrite = TRUE)
    plot(probbin, png(paste0("../output/maxent/hotspot_mapping/prob_by_depth/", mth, "_maxkap_", ascname, ".png")), overwrite = TRUE)
    dev.off() # stops automatic saving of the plot to a png
  }
}
...
filelist <- list.files("../output/maxent/paper_plots/hotspot_mapping/", pattern = "maxkap_single.asc", include.dirs = TRUE)

x <- stack(file.path("../output/maxent/paper_plots/hotspot_mapping/",filelist)) %>% 
  mask(mcp) %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
  gather(key = "variable", value = "value",-x,-y) %>% 
...
#quickest way might be just to inject my csv file with the correct values here....
df2 <- read.csv("../output/maxent/paper_plots/cell_count_maxkap.csv", header = TRUE)
df$total <- df2$proportion
df$total <- format(round(df$total, digits = 2))
#colnames(df) <- c("month", "cellc", "total")
...
ggsave("../output/maxent/paper_plots/maxkap_2d.png", height = 210, width = 297, units = "mm", dpi = 600)

```


e maxkappa threshold
```{r}
binprob <- function(x){
  ifelse(x > 0 & x >= mk, 1,
  ifelse(x <= 0, 0, NA))
...
then create an summary map....
```{r >maxkap prob monthly 2D representation}
mthord <- c("mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")

for (m in 1:length(mthord)){
  mthna <- mthord[m]
  binaryprob_list <- list.files(paste0("../output/maxent/paper_plots/hotspot_mapping/"), pattern = paste0(mthna, "*_maxkap_.*.asc$"), full.names = TRUE)
  binstack <- stack(binaryprob_list)
  stacksum <- sum(binstack, na.rm = TRUE)
  writeRaster(stacksum, paste0("../output/maxent/paper_plots/hotspot_mapping/", mthna, "_maxkap_2d.asc"), overwrite=TRUE)
  plot(stacksum, png(paste0("../output/maxent/paper_plots/hotspot_mapping/", mthna, "_maxkap_2d.png")))
  dev.off()
}
...
binprob <- function(x){
  ...
  clogasc_list <- list.files("../output/maxent/paper_plots/hotspot_mapping", pattern = "*_maxkap_2d.asc", full.names = TRUE)
  for (cum in 1:length(clogasc_list)){
    cumasc <- raster(clogasc_list[cum])
    ascname <- str_sub(clogasc_list[cum], start = 46, end = -8)
    probbin <- calc(cumasc, fun = binprob) #creates raster layer with cell values based on the binprob function
    writeRaster(probbin, paste0("../output/maxent/paper_plots/hotspot_mapping/", ascname, "_single.asc"), overwrite = TRUE)
    plot(probbin, png(paste0("../output/maxent/paper_plots/hotspot_mapping/", ascname, "_single.png")), overwrite = TRUE)
    dev.off() # stops automatic saving of the plot to a png
...
now create a table to see how many cells there are >maxkap in each month (for editing plots)

```{r count cells >maxkap}

binaryprob_list <- list.files("../output/maxent/paper_plots/hotspot_mapping", pattern = ".*_maxkap_2d.asc", full.names = TRUE)
sum2d <- data.frame(matrix(ncol = 2, nrow = 10))
sum2d <- setNames(sum2d, c("month", "count"))
...
for (r in 1:length(binaryprob_list)){
  ras <- raster(binaryprob_list[r])
  rnm <- names(ras)
  sum2d$month[r] <- gsub("_maxkap_2d", "", x = rnm)
  sum2d$count[r] <- cellStats(ras, sum, asSample = TRUE)
}
...
write.csv(sum2d, "../output/maxent/paper_plots/cell_count_maxkap.csv", row.names = FALSE)
...
get data to plot maxkap binary by depth... 
```{r}
mthord <- c("mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")
...
for (m in 1:length(mthord)){
  mth <- mthord[m]
  ascdepth <- list.files(paste0("../output/maxent/paper_plots/prob_by_depth"), pattern = paste0("^", mth, "_maxkap.*asc"), full.names = TRUE)
  ...
  for (lyr in 1:nlayers(binaryavgstack)){
    ascname <-  str_sub(ascdepth[lyr], 67, -5)
    dlay <- binaryavgstack[[lyr]]
    dlay <- calc(dlay, fun = binprob) #creates raster layer with cell values based on the binprob function
    lyrsum <- cellStats(dlay, sum)
    stackstathigh$prob_bin[lyr] <- lyrsum
  ...
  write.csv(stackstathigh, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_cellcount_bydepth_avg.csv"), row.names = FALSE)
}
...
```{r percentage cells above and below maxkap}
mthord <- c("mar",  "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")
cumst1 <- read.csv("../output/maxent/paper_plots/cellsperlayer.csv", header = TRUE)

for (m in 1:length(mthord)){
  mth <- mthord[m]
  a <- read.csv(paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_cellcount_bydepth_avg.csv"), header = TRUE, stringsAsFactors=FALSE)
  for (r in 1:nrow(a)){
    ...
    a$p_cells[r] <- (a$prob_bin[r]/a$total_cells[r])*100
    a$Probability[r] <- 1
  }
    #a$asclayer <- as.numeric(a$asclayer)
  
  a2 <- as.data.frame(a$asclayer)
  a2$total_cells <- a$total_cells
  a2$prob_bin <- a$total_cells - a$prob_bin
  a2$p_cells <- (a2$prob_bin/a2$total_cells)*100
  a2$Probability <- 0
  colnames(a2)[1] <- "asclayer"
  
  b <- rbind(a, a2)
  colnames(b)[1] <- "Depth"


  write.csv(b, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_dis_prob_cells.csv"), row.names = FALSE)
}
...
based on Remi's code - maxkappa derrived habitat by depth
...
filenames <- list.files("../output/maxent/paper_plots/prob_by_depth/", pattern = "_maxkap_dis_prob_cells.csv", recursive = TRUE,full.names = TRUE)

all <- map_dfr(filenames, function(x) fread(x) %>% 
                       mutate(Month=substr(basename(x),1,3))) %>% 
  mutate(Depth = factor(round(Depth,1)),
         Depth = factor(Depth,ordered = TRUE, levels = rev(levels(Depth))),
         Month=factor(months(parse_date_time(Month,"%m")),
                      ordered = TRUE,
                      levels=months(parse_date_time(1:12,"%m"))))
...
#make probability a factor - sam added this
all$Probability <- as.factor(all$Probability)
...
#make the plot
  ...
  labs(y = "% Ocean at Depth", x = "Depth (meters)")+
  scale_fill_manual(aesthetics = "fill", values=c('light yellow','black'))+
  theme_classic()+
  theme(legend.position = "bottom", axis.text.y = element_text(size=7), strip.background = element_blank()) +
  scale_x_discrete(breaks = c(dlst[1], dlst[5], dlst[10], dlst[15], dlst[20], dlst[25], dlst[30], dlst[35], dlst[40], dlst[47]))

ggsave("../output/maxent/paper_plots/maxkap_prob_by_depth.png", width = 210, height = 297, units = "mm", dpi = 600)
```


#get max and min depth layers based on max kappa
```{r}
kapd <- list.files("../output/maxent/paper_plots/prob_by_depth/" , pattern = "_maxkap_dis_prob_cells.csv", full.names = TRUE)
kapmmd <- data.frame(mth = character(10), min = numeric(10), max = numeric(10))
kapmmd$mth <- as.character(kapmmd$mth)

for(k in 1:length(kapd)){
  kapmmd$mth[k] <- str_sub(kapd[k], start = 44, end = 46)
  kd <- read.csv(kapd[k])
  kd <- subset(kd, Probability == 1) #just keep the presence cells
  kd <- subset(kd, prob_bin > 0) # get the depths where there are presence cells
  kd <- kd[order(kd[, 1]),] #order by depth 
  kapmmd$min[k] <- kd$Depth[1] #minimum depth of presence cell
  kapmmd$max[k] <- tail(kd$Depth, n = 1) #maximum depth of presence cell
}

write.csv(kapmmd, "../output/maxent/paper_plots/hotspot_mapping/kappa-min-max-depth.csv")
```

#get lat long position of most northely, southerly, western, and eastern cell centroid

```{r}
filelist <- list.files("../output/maxent/paper_plots/hotspot_mapping/", pattern = "maxkap_single.asc", full.names = TRUE)
lonlat <- data.frame(mth = character(10), west = numeric(10), east = numeric(10), north = numeric(10), south = numeric(10))
lonlat$mth <- as.character(lonlat$mth)


for (k in 1:length(filelist)){
  ras <- raster(filelist[9])
  crs(ras) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0" #the projection of the raster
  lonlat$mth[k] <- str_sub(filelist[k], start = 46, end = 48)
  cellloc <- rasterToPoints(ras, fun=function(x){x==1}) #make raster to point only for cells with the value of 1 (i.e. presence cells)
  cellloc <- as.data.frame(cellloc) # x = lon, y = lat
  cellloc <- cellloc[order(cellloc$x), ] #order dataframe by lon from low (west) to high (east)
  lonlat$west[k] <- cellloc$x[1] #most western cell centroid
  lonlat$east[k] <- tail(cellloc$x, n = 1) #most eastern cell centroid
  cellloc <- cellloc[order(cellloc$y), ] #order dataframe by lat from low (south) to high (north)
  lonlat$south[k] <- cellloc$y[1] #most southern cell centroid
  lonlat$north[k] <- tail(cellloc$y, n = 1) #most northern cell centroid
}



write.csv(lonlat, "../output/maxent/paper_plots/hotspot_mapping/kappa-nswe.csv")

```

#get lat long position of most northely, southerly, western, and eastern cell centroid for oct, june, aug, may (to remove northern spots)
...
for (k in 1:length(filelist)){
  ras <- raster(filelist[k])
  crs(ras) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0" #the projection of the raster
  lonlat$mth[k] <- str_sub(filelist[k], start = 46, end = 48)
  cellloc <- rasterToPoints(ras, fun=function(x){x==1}) #make raster to point only for cells with the value of 1 (i.e. presence cells)
  cellloc <- as.data.frame(cellloc) # x = lon, y = lat
  cellloc <- subset(cellloc, y <55) #remove all the northern points
  cellloc <- cellloc[order(cellloc$y), ] #order dataframe by lat from low (south) to high (north)
  lonlat$south[k] <- cellloc$y[1] #most southern cell centroid
  lonlat$north[k] <- tail(cellloc$y, n = 1) #most northern cell centroid
  cellloc <- cellloc[order(cellloc$x), ] #order dataframe by lon from low (west) to high (east)
...
write.csv(lonlat, "../output/maxent/paper_plots/hotspot_mapping/kappa-nswe_nonorth.csv")

```

```{r}
t1 <- distVincentyEllipsoid(c(0,0),c(90,90))
# on a 'Clarke 1880' ellipsoid
test <- distVincentyEllipsoid(c(0,0),c(90,90), a=6378249.145, b=6356514.86955, f=1/293.465)
```

#background point selection plots

```{r}
test_back_df <- read.csv("../data/env/background_point_check/201406_allbackgroundpoints.csv", header = TRUE)
```
...
make long data
```{r}
bkpt <- data.frame(no_points = character(370000), variable = character(370000), value = numeric(370000))
bkpt$variable <- as.character(bkpt$variable)
bkpt$no_points <- as.character(bkpt$no_points)

bkpt$no_points[1:10000] <- "10,000"
bkpt$no_points[10001:30000] <- "20,000"
bkpt$no_points[30001:80000] <- "50,000"
bkpt$no_points[80001:180000] <- "100,000"
bkpt$no_points[180001:370000] <- "190,000"
bkpt$variable <- "temp"
bkpt$value[1:10000] <- testbk10000$temp_depth
bkpt$value[10001:30000] <- testbk20000$temp_depth
bkpt$value[30001:80000] <- testbk50000$temp_depth
bkpt$value[80001:180000] <- testbk100000$temp_depth
bkpt$value[180001:370000] <- testbk190000$temp_depth


bkpt2 <- data.frame(no_points = numeric(370000), variable = character(370000), value = numeric(370000))
bkpt2$variable <- as.character(bkpt2$variable)
bkpt2$no_points[1:10000] <- "10,000"
bkpt2$no_points[10001:30000] <- "20,000"
bkpt2$no_points[30001:80000] <- "50,000"
bkpt2$no_points[80001:180000] <- "100,000"
bkpt2$no_points[180001:370000] <- "190,000"
bkpt2$variable <- "salinity"
bkpt2$value[1:10000] <- testbk10000$salinity_depth
bkpt2$value[10001:30000] <- testbk20000$salinity_depth
bkpt2$value[30001:80000] <- testbk50000$salinity_depth
bkpt2$value[80001:180000] <- testbk100000$salinity_depth
bkpt2$value[180001:370000] <- testbk190000$salinity_depth

bkpt3 <- data.frame(no_points = numeric(370000), variable = character(370000), value = numeric(370000))
bkpt3$variable <- as.character(bkpt3$variable)
bkpt3$no_points[1:10000] <- "10,000"
bkpt3$no_points[10001:30000] <- "20,000"
bkpt3$no_points[30001:80000] <- "50,000"
bkpt3$no_points[80001:180000] <- "100,000"
bkpt3$no_points[180001:370000] <- "190,000"
bkpt3$variable <- "o2"
bkpt3$value[1:10000] <- testbk10000$o2_depth
bkpt3$value[10001:30000] <- testbk20000$o2_depth
bkpt3$value[30001:80000] <- testbk50000$o2_depth
bkpt3$value[80001:180000] <- testbk100000$o2_depth
bkpt3$value[180001:370000] <- testbk190000$o2_depth

bkpt4 <- data.frame(no_points = numeric(370000), variable = character(370000), value = numeric(370000))
bkpt4$variable <- as.character(bkpt4$variable)
bkpt4$no_points[1:10000] <- "10,000"
bkpt4$no_points[10001:30000] <- "20,000"
bkpt4$no_points[30001:80000] <- "50,000"
bkpt4$no_points[80001:180000] <- "100,000"
bkpt4$no_points[180001:370000] <- "190,000"
bkpt4$variable <- "chl"
bkpt4$value[1:10000] <- testbk10000$chl_surface
bkpt4$value[10001:30000] <- testbk20000$chl_surface
bkpt4$value[30001:80000] <- testbk50000$chl_surface
bkpt4$value[80001:180000] <- testbk100000$chl_surface
bkpt4$value[180001:370000] <- testbk190000$chl_surface


bk <- rbind(bkpt, bkpt2, bkpt3, bkpt4)
bk$no_points <- as.factor(bk$no_points)
```

```{r}
ggplot(bk, aes(x = value)) +
    stat_density(aes(group = no_points, color = no_points),position="identity",geom="line") +
  facet_wrap(~variable,ncol=2)
  
```

problem here is that i don't want the axis shared

fiddle with the dataset again
```{r}
t <- subset(bk, variable == "temp")
t$no_points <- factor(t$no_points, levels = c("10,000", "20,000", "50,000", "100,000", "190,000"))
s <- subset(bk, variable == "salinity")
s$no_points <- factor(s$no_points, levels = c("10,000", "20,000", "50,000", "100,000", "190,000"))
o <- subset(bk, variable == "o2")
o$no_points <- factor(o$no_points, levels = c("10,000", "20,000", "50,000", "100,000", "190,000"))
c <- subset(bk, variable == "chl")
c$no_points <- factor(c$no_points, levels = c("10,000", "20,000", "50,000", "100,000", "190,000"))

```

plot
```{r}
p1 <- ggplot(t, aes(x = value)) + stat_density(aes(group = no_points, color = no_points),position="identity",geom="line", size=0.75) +theme_classic(base_size = 18) + labs(x = "Temperature (Kelvin)", y = "Density") + guides(color = guide_legend(title = "No Points"))
p2 <- ggplot(s, aes(x = value)) + stat_density(aes(group = no_points, color = no_points),position="identity",geom="line", size=0.75) + theme_classic(base_size = 18) + labs (x = "Salinity (PSU)", y = "Density") + guides(color = guide_legend(title = "No Points"))
p3 <- ggplot(o, aes(x = value)) + stat_density(aes(group = no_points, color = no_points),position="identity",geom="line", size=0.75) +theme_classic(base_size = 18) + labs (x = "Dissolved Oxygen (mmol\u207B\u00B3)", y = "Density") + guides(color = guide_legend(title = "No Points"))
p4 <- ggplot(c, aes(x = value)) + stat_density(aes(group = no_points, color = no_points),position="identity",geom="line", size=0.75) + theme_classic(base_size = 18) + labs (x = "Chlorophyll \u03b1  (mmol\u207B\u00B3)", y = "Density") + guides(color = guide_legend(title = "No Points"))

p <- ggarrange(p1, p2, p3, p4, ncol = 2, nrow = 2, common.legend = TRUE, legend = "bottom")

ggsave("../output/maxent/paper_plots/bkpt_201406.png", width = 210, height = 297, units = "mm", dpi = 600)
...
```{r}
test_back_df <- read.csv("../data/env/background_point_check/200710_allbackgroundpoints.csv", header = TRUE)
```


ok now create the first lot of random for 200710
...
ggsave("../output/maxent/paper_plots/bkpt_200710.png", width = 210, height = 297, units = "mm", dpi = 600)

```
199902

```{r}
test_back_df <- read.csv("../data/env/background_point_check/199902_allbackgroundpoints.csv", header = TRUE)
```
...
ggsave("../output/maxent/paper_plots/bkpt_199902.png", width = 210, height = 297, units = "mm", dpi = 600)

```

#plot for prediction method in appendix
...
```{r plot sample depths}
proj <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
crs(s) <- proj

temp9 <- raster("../data/env/asciitrim/ascii2004/2004_07_temp.nc_9.82275.asc")
crs(temp9) <- proj
temp221 <- raster("../data/env/asciitrim/ascii2004/2004_07_temp.nc_221.141.asc")
crs(temp221) <- proj
temp508 <- raster("../data/env/asciitrim/ascii2004/2004_07_temp.nc_508.64.asc")
crs(temp508) <- proj

ts <- stack(temp9, temp221, temp508)

levelplot(ts, layout = c(1, 3), scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c("", "", ""), colorkey=FALSE, margin = FALSE, par.setting = viridisTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/temp-example.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))

salinity9 <- raster("../data/env/asciitrim/ascii2004/2004_07_salinity.nc_9.82275.asc")
crs(salinity9) <- proj
salinity221 <- raster("../data/env/asciitrim/ascii2004/2004_07_salinity.nc_221.141.asc")
crs(salinity221) <- proj
salinity508 <- raster("../data/env/asciitrim/ascii2004/2004_07_salinity.nc_508.64.asc")
crs(salinity508) <- proj

ts <- stack(salinity9, salinity221, salinity508)

levelplot(ts, layout = c(1, 3), scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c("", "", ""), colorkey=FALSE, margin = FALSE, par.setting = viridisTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/salinity-example.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))
          
```

```{r plot env concat}
ras <- raster("../output/maxent/paper_plots/bent_illus/tempmerge.asc")
levelplot(ras, scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c(""), colorkey=FALSE, margin = FALSE, par.setting = viridisTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/temp-concat.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))

ras <- raster("../output/maxent/paper_plots/bent_illus/salmerge.asc")
levelplot(ras, scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c(""), colorkey=FALSE, margin = FALSE, par.setting = viridisTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/sal-concat.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))
```

```{r plot concat predictions}
ras <- raster("../output/maxent/paper_plots/bent_illus/predmerge.asc")
levelplot(ras, scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c(""), colorkey=FALSE, margin = FALSE, par.setting = magmaTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/pred-concat.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))
```

```{r plot split predictions}
proj <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"

pred9 <- raster("../output/maxent/paper_plots/bent_illus/pred/cloglog_2004_07_9.82275.asc")
crs(pred9) <- proj
pred221 <- raster("../output/maxent/paper_plots/bent_illus/pred/cloglog_2004_07_221.141.asc")
crs(pred221) <- proj
pred508 <- raster("../output/maxent/paper_plots/bent_illus/pred/cloglog_2004_07_508.64.asc")
crs(pred508) <- proj

ts <- stack(pred9, pred221, pred508)

levelplot(ts, layout = c(1, 3), scales=list(draw=FALSE), xlab = NULL, ylab = NULL, names.attr = c("", "", ""), colorkey=FALSE, margin = FALSE, par.setting = magmaTheme)
dev.copy(png, (paste0("../output/maxent/paper_plots/bent_illus/pred-example.png")), width = 1080,height = 1920)
dev.off() # stops automatic saving of the plot to a png))
```

#table 2 made as a plot

gather all the percentages in a single file
```{r}
flst <- list.files("./output/maxent/paper_plots/prob_by_depth/", pattern = "_cellprob_totals.csv", full.names = TRUE)
ptables <- lapply(flst, read.csv, header = TRUE)
pall <- do.call(rbind, ptables)
pall$month <- capitalize(pall$month)

ggplot(data = pall, aes(x = month, y = percent, fill = Category)) + geom_bar(stat = "identity")
```

# find mean bottom depth of study region

```{r}
bathyrast <- raster("./data/env/bathymetric_glorys.asc") 
plot(bathyrast)
```
need to reclassify cells outside the mcp to NA 
```{r}
outsidemcp <- shapefile("./data/env/landp/outside-mcp-wgs84.shp") 
ascdirs <- list.dirs("./data/env/ascii", full.names = FALSE, recursive = FALSE) 

ras<- bathyrast
ras[outsidemcp] <- 500000
ras[ras == 500000] <- NA
writeRaster(ras, "./data/env/bathy_trimmed.asc", overwrite=TRUE)
cellStats(ras, stat = "mean", na.rm = TRUE) #1046.574
summary(ras) #313.5291
```

find the number of folds in each month with a TSS <0.6
```{r}
flst <- list.files("./output/maxent/models/test-comb", pattern = "avgAIC.csv", full.names = TRUE, recursive = FALSE)
df <- data.frame(month=character(10), nfolds=numeric(10), stringsAsFactors = FALSE)

for (f in 1:length(flst)){
  af <- read.csv(flst[f])
  af <- head(af, -1)
  mth <- (sapply(strsplit(flst[f], "/"), "[[", 6)) 
  df$month[f] <- stringr::str_sub(mth, end = 3)
  df$'x<0.6'[f] <- sum(af$tss <= 0.6)
  df$nfolds[f] <- nrow(af)
}
write.csv(df, "./output/maxent/models/test-comb/tssfoldcount.csv", row.names = FALSE)
```



boxplots of depth of samples relatvie to the bottom depth
```{r}
presback <- read.csv("./output/bio/presab_nafo_yr.csv", header = TRUE) 
pres <- subset(presback, occurrence == 1)
pres <- subset(pres, select = c(month, bottom_depth_glorys, depth_layer))
pres$depthdiff <- pres$bottom_depth_glorys - pres$depth_layer
pres <- subset(pres, month >2)
pres$month <- as.factor(pres$month)

ggplot(pres, aes(x = month, y = depth_layer)) + geom_boxplot() + scale_y_reverse() + labs(y = "sampling depth")

ggplot(pres, aes(x = month, y = depthdiff)) + geom_boxplot() + labs(y = "sampling depth relative to sea floor (0)")
...
for (m in 1:length(mthord)){
  ...
  write.csv(stackstathigh, paste0("../output/maxent/paper_plots/prob_by_depth/", mth, "_maxkap_", bin, "_cellcount_bydepth_avg.csv"), row.names = FALSE)
  }
}
...
plot - Code base provided by Rémi Daigle (https://remidaigle.wordpress.com/)
...
filenames <- list.files("./output/maxent/paper_plots/prob_by_depth/", pattern = "dis_prob_cells.csv", recursive = TRUE,full.names = TRUE)
filenames <- filenames[ !grepl("maxkap", filenames) ] #remove all the maxkap files
...
#make the plot
  ...
  theme_classic()+
  theme(legend.position = "bottom", axis.text.y = element_text(size=7), strip.background = element_blank()) +
  scale_x_discrete(breaks = c(dlst[1], dlst[5], dlst[10], dlst[15], dlst[20], dlst[25], dlst[30], dlst[35], dlst[40], dlst[47])) + geom_vline(xintercept=12.5, linetype = 2, col = "light grey") # this is approximate
  
ggsave("./output/maxent/paper_plots/prob_by_depth_line.png", width = 210, height = 297, units = "mm", dpi = 600)
```


get data to plot maxkap binary by depth plot (Figure 6)
...
#make the plot
  ...
  facet_wrap(~Month,ncol=2)+
  labs(y = "% Ocean at Depth", x = "Depth (meters)")+
  scale_fill_manual(aesthetics = "fill", values=c('light yellow','black'), name = "Presence")+
  theme_classic()+
  theme(legend.position = "bottom", axis.text.y = element_text(size=7), strip.background = element_blank()) +
  scale_x_discrete(breaks = c(dlst[1], dlst[5], dlst[10], dlst[15], dlst[20], dlst[25], dlst[30], dlst[35], dlst[40], dlst[47])) +
    geom_vline(xintercept=12.5, linetype = 2, col = "light grey") # this is approximate

ggsave("./output/maxent/paper_plots/maxkap_prob_by_depth_line.png", width = 210, height = 297, units = "mm", dpi = 600)
```


This is Figure 3

Code base provided by Rémi Daigle (https://remidaigle.wordpress.com/)

have moved sample files into a separate folder
march,  june, sep, dec
11.7737, 61.1128, 199.79. 628.026
...
df$month <- sapply(df$month, CapStr)
df$month_f = factor(df$month, levels=c('Mar','Jun','Sep','Dec'))
...
ggsave("./output/maxent/paper_plots/prob-overview_swap.png", width = 210, height = 297, units = "mm", dpi = 600)
```
