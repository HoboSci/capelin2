...
mthn <- c("jan", "mar", "apr", "may", "jun", "july", "aug", "sep", "oct", "nov","dec")
...
for (i in 1:length(avgasc_list)) {  
 de <- data_aea_sp$depthlyr_mod[j]  # a variable for the observation depth layer
  ...
  rasdep <- str_sub(avgasc_list[i], 48, -5)
...
write.csv(data_aea1, "../output/maxent/all_data/testforslr_jul.csv", row.names = FALSE)
 #}
