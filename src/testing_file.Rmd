...
cumasc <- raster::mask(cumasc, outsidemcp, inverse = TRUE)
cumasc2 <- raster::mask(cumasc2, outsidemcp, inverse = TRUE)
ss <- raster::mask(s, outsidemcp, inverse = TRUE) #HERE NEED TO WORK OUT HOW TO CLIP LAND


pstack <- stack(cumasc, cumasc2)

my.at <- seq(0, 1, by = 0.1)
mapTheme <- viridisTheme(region = viridis(10))



p <- levelplot(pstack, ylab = NULL, xlab = NULL, scales=list(draw=FALSE), colorkey=list(space="bottom", at = my.at), par.settings = mapTheme, layout = c(2, 1), names.attr = c("6m", "180m"), margin = FALSE)
p + layer(sp.polygons(s, fill = "black")) 
cumasc <- raster::mask(p, outsidemcp, inverse = TRUE)
