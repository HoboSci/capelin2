...
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps", "lattice", "tidyr", "enmSdm", "ggplot2", "plyr")
source("../src/ipak.R")
ipak(packages)
```
...
start with mpa binary map first
...
for(m in 1:length(mthlst)){
  ...
  write.csv(depthlayers, (paste0("../output/maxent/predictions/", mthna, "_binary_avg_asclayerbydepth.csv")), row.names = TRUE)
}


a <- read.csv("../output/maxent/predictions/apr_binary_avg_asclayerbydepth.csv", header = TRUE)
b <- read.csv("../output/maxent/predictions/may_binary_avg_asclayerbydepth.csv", header = TRUE)
c <- merge(a, b, by=0)

data <- rbindlist(lapply(list.files("../output/maxent/predictions/", pattern = ".*binary_avg_asclayerbydepth.csv", full.names = TRUE), fread))
