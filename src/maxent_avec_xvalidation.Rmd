...
attach depth in meters (layer_name in depth_bin.csv) to the stackstat based on layer (stackstat) and depthlayerno (depth_bin.csv)

```{r}
dep <- read.csv("../data/env/depth_bin.csv", header = TRUE) # the depth bin file created way back
dep <- subset(dep, select = c("depthlayerno", "layer_name"))
stackstat <-  read.csv(paste0("../output/maxent/predictions/", mthna, "/cellcount_bydepth_avg.csv"), header = TRUE)
colnames(dep)[names(dep) == "depthlayerno"] <- "layer"
stackstat <- left_join(stackstat, dep)
stackstat[ , "layer_name"]=round(stackstat[ , "layer_name"], 2)
head(stackstat)
