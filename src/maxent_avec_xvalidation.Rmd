...
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps", "lattice", "tidyr")
source("../src/ipak.R")
ipak(packages)
```
...
binaryclog_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^binary_.*.asc", full.names = TRUE)
...
for (binyr in (1:length(y_list))){
  ...
  writeRaster(stacksum, paste0("../output/maxent/predictions/", mthna, "/binary_2d_", year, ".asc"))
...
clogasc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^cloglog_.*.asc", full.names = TRUE)
stdep <- read.csv(paste0("../output/maxent/predictions/", mthna, "/stack_depth.csv"), header = TRUE)
...
stacksum <- sum(binstack, na.rm = TRUE)
writeRaster(stacksum, paste0("../output/maxent/predictions/", mthna, "/binary_avg_2d.asc"))
...
ascdepth_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^binary_avg.*asc", full.names = TRUE)
...
bydepth <- barchart(layer ~ cell_sum, data = stackstat, ylim = c(49, 1), xlim = c(0, 300))
...
#cbi
use enmSdm package? see exercise 10...
positive values are good!

should move this up to evaluation....

```{r}

for (a in 1:length(modelmonth)){
  timeslice <- modelmonth[a]
  yr <- str_sub(timeslice, -4) #the year being used for testing
  predPres <- read.csv(paste0("../output/maxent/models/", a, "/species_samplePredictions.csv"), header = TRUE)
  predBg <- read.csv(paste0("../output/maxent/models/", a, "/species_backgroundPredictions.csv"), header = TRUE)
...
predPres <- read.csv("../output/maxent/models/jan2003/species_samplePredictions.csv", header = TRUE)
predBg <- read.csv("../output/maxent/models/jan2003/species_backgroundPredictions.csv", header = TRUE)
predPres <- predPres$Cloglog.prediction
predBg <- predBg$Cloglog
cbi <- contBoyce(pres = predPres, bg = predBg)
cbi
