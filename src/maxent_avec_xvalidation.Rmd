...
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps", "lattice")
source("../src/ipak.R")
ipak(packages)
```
...
for (year in 1:nrow(modelmonth)){
  ...
  if (yr == "2003" | yr == "2004"){
    ...
    evsensitivity <- threshold(ev, 'sensitivity')
      ...
      aicetc$test_coefficients <- nocoeff #this is to calculate the AIC
      aicetc$test_sum.of.logs <- sumsamplepred #this is to calculate the AIC
   ...
   for (td in 1:nrow(unlayer_yr)){
      ...
      stack <- stack(temp_depth, salinity_depth, o2_depth, chl_surface, nao_sample, nao_winter)
...
for some reason the cumulative output comes empty... using bossMaps
```{r raw to cumulative}
rawasc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^raw_.*.asc", full.names = TRUE) 
for (raw in 1:length(rawasc_list)){
  asc <- raster(rawasc_list[raw])
  ascname <- names(asc)
  ascname <- str_sub(ascname, start = 5)
  cumasc <- cumulative(asc, threshold = NULL)
  writeRaster(cumasc, paste0("../output/maxent/predictions/", mthna, "/", "cum_", ascname, ".asc"), overwrite = TRUE)
}
```
...
```
cum_mpathr <- calc(cumasc, fun = rc)
plot(cum_mpathr)
tenper_thr <- 20.338725


```{r binary maps on individual layers}
cumasc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^cum_.*.asc", full.names = TRUE)
...
for (modtime in 1:nrow(modelmonth)){
  ...
  mpa_thr <- thresholds$mpa_0.9_cum
  print(mpa_thr)
  for (cum in 1:length(cumasc_list)){
    cumascyr <- str_sub(cumasc_list[cum], start = 40, end = 43) #selects the year from the file name
    if(yr == cumascyr){
      cumasc <- raster(cumasc_list[cum])
      ascname <- names(cumasc)
      print(ascname)
      cum_mpathr <- calc(cumasc, fun = binmpa)
      mpa_plot <- plot(cum_mpathr, png(paste0("../output/maxent/predictions/", mthna, "/", "binary_mpa_", ascname, ".png")))
      dev.off() # stops automatic saving of the plot to a png
      writeRaster(cum_mpathr, paste0("../output/maxent/predictions/", mthna, "/", "binary_mpa_", ascname, ".asc"), overwrite = TRUE)
    }
  }
}
...
```{r avg asc layers cumulative}
asc_list <- list.files(paste0("../output/maxent/predictions/", mthna, "/"), pattern = "^cum.*asc", full.names = TRUE)
depthlayers <- data.frame(cbind(unique(stdep$ascdepths)))
names(depthlayers)[names(depthlayers) == "cbind.unique.stdep.ascdepths.."] <- "depth"
depthlayers$depth <- as.character(depthlayers$depth)
...
write.csv(depthlayers, (paste0("../output/maxent/predictions/", mthna, "/cum_asclayerbydepth.csv")), row.names = TRUE)
...
for (x in 1:nrow(depthlayers)){
  ...
  writeRaster(avg, paste0("../output/maxent/predictions/", mthna, "/avg_cum_", asclaydep, ".asc"), overwrite=TRUE)
  avgplot <- plot(avg, png(paste0("../output/maxent/predictions/", mthna, "/avg_cum_", asclaydep, ".png"))) 
  dev.off()
}
...
```{r binary maps on averaged rasters}
avgcumasc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^avg_cum.*.asc", full.names = TRUE)
thresholds <- read.csv(paste0("../output/maxent/models/", mthna, "avgAIC.csv"), header = TRUE)
thresholds <- tail(thresholds, 1) #subsset the last row
mpa_thr <- thresholds$mpa_0.9_cum
#print(mpa_thr)

for (avgcum in 1:length(avgcumasc_list)){
    cumasc <- raster(avgcumasc_list[avgcum])
    ascname <- names(cumasc)
    #print(ascname)
    cum_mpathr <- calc(cumasc, fun = binmpa)
    mpa_plot <- plot(cum_mpathr, png(paste0("../output/maxent/predictions/", mthna, "/", "binary_mpa_", ascname, ".png")))
    dev.off() # stops automatic saving of the plot to a png
    writeRaster(cum_mpathr, paste0("../output/maxent/predictions/", mthna, "/", "binary_mpa_", ascname, ".asc"), overwrite = TRUE, datatype="LOG1S")
}
...
ascdepth_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^binary_mpa_avg_cum_.*asc", full.names = TRUE)
...
bydepth <- barchart(layer ~ cell_sum, data = stackstat, ylim = c(49, 1), xlim = c(1, 1600))
trellis.device(device="png",filename = paste0("../output/maxent/predictions/", mthna, "/", mthna, "_depthplot.png"))
print(bydepth)
dev.off()
```

what about the variation from one year to the next

e.g. binary_mpa_st200401041.18.asc

```{r no presence cells per layer per year}
binaryyr_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^binary_mpa_st.*asc", full.names = TRUE)
yrlayer <- data.frame(matrix(ncol = 2, nrow = length(binaryyr_list)))
colnames(yrlayer) <- c("layer", "year")



for (yrlyr in 1:length(binaryyr_list)){
  yearoflyr <- str_sub(binaryyr_list[yrlyr], start = 47, end = 50) #selects the year from the file name
  yrlayer$year[yrlyr] <- yearoflyr
  yrlayer$layer[yrlyr] <- binaryyr_list[yrlyr]
}
...
stackstat_yr <- data.frame(matrix(ncol = length(noyrs) + 1, nrow = nlayers(binaryavgstack)))
stackstat_yr[ ,1] <- c(1:49)
colnames(stackstat_yr) <- c("layer", paste0("y_", noyrs))

for (binyr in (1:length(y_list))){
  lsta <- stack(y_list[[binyr]]$layer)
  yr <- y_list[[binyr]]$year[1]
  for (l in 1:nlayers(lsta)){
    lyrsum <- cellStats(lsta[[l]], sum)
    stcol <- paste0("y_", yr) 
    stackstat_yr[[l, stcol]] <- lyrsum
  }
}

#2005 coming up empty...
