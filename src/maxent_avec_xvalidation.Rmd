...
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps")
source("../src/ipak.R")
ipak(packages)
```
...
now to stack average binary maps at depth into one 2D representation...

```{r}

#this below is to get an average score for all layers per year. probably not what you want
...
stsum <- sum(binaryavgstack, na.rm = TRUE)
  writeRaster(stsum, (paste0("../output/maxent/predictions/", mthna, "/binary_sum_", mthna, ".asc")), overwrite=TRUE)
  sumplot <- plot(stsum, png(paste0("../output/maxent/predictions/", mthna, "/binary_sum_", mthna, ".png"))) 
  dev.off()
stavg <- calc(binaryavgstack, fun = mean, na.rm = TRUE)
  writeRaster(stavg, (paste0("../output/maxent/predictions/", mthna, "/binary_avg_", mthna, ".asc")), overwrite=TRUE)
  avgplot <- plot(stavg, png(paste0("../output/maxent/predictions/", mthna, "/binary_avg_", mthna, ".png"))) 
  dev.off()
```
...
```{r}
test <- cellStats(binaryavgstack[[30]], sum)
test
```
