...
New package attempt to do this.... #include=FALSE

```{r pre-install & load packages, }
install.packages("pacman")
library(pacman)
p_load(raster, dplyr, data.table, sp, adehabitatHR, maptools)
...
ok yea, i guess.... still wondering about those edge areas....anyway
...
```{r}
sp_background <- data.frame(test, long=coordinates(sp_background)[,1], lat=coordinates(sp_background)[,2])
head(sp_background)
```

change name of long lat columns to decimalLongitude and decimalLatitude respectively
```{r}
sp_background <- as.data.frame(sp_background) #need to go back to a 'normal' dataframe to change col names
head(sp_background) #note the columns in question have changed name...
colnames(sp_background)[colnames(sp_background)=="longitude.1"] <- "decimalLongitude"
colnames(sp_background)[colnames(sp_background)=="latitude.1"] <- "decimalLatitude"
head(sp_background)
```
...
```{r}
plot(map2, xlim = c(-65, -40), ylim =c(30, 70), asp = 1, main = "Occurences missing a NAFO Zone", col = "cornsilk") #the x and y lim are the long-lat bounds of the map
...
huh that is interesting... I think I want to plot this in Arc and take a closer look...
