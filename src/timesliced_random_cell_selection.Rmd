...
```
```{r}
background_all <- merge(x = background_all, y = depthbin[ , c("depthlayerno", "depth_layer")], by.x = ("depthlayerno"), all.x = TRUE)
head(background_all)
```
...
```{r}
background_all$bottom_depth <- get.depth(bat, background_all[ ,10:11], locator = FALSE)
...
```{r}
names(background_all)[names(background_all)=="freq"] <- "total_cell_obs"
head(background_all)
```
...
names(background_all)[names(background_all)=="freq"] <- "yymm_cell_obs"
names(background_all)[names(background_all)=="freq"] <- "yymmdepth_cell_obs"
