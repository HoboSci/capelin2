...
for (i in 1:no_timeslices){
  ...
  write.csv(time_cell, paste0(names(lapply(split_cell_yymm, names))[i], ".csv"), row.names = FALSE) 
}
```
...
```{r}
library(dplyr)
test_cell_selection <- read.csv("../output/bio/background/1998.10.csv", header = TRUE) #just load one csv to play with
test_ran_cell_sel <- sample_n(test_cell_selection, 10) #where 10 = number of rows to sample
```
