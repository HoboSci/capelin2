...
```{r}
test_timeslice <- split_cell_yymm[[1]]
test_timeslice
test_timeslice <- as.data.frame(test_timeslice)

timeslice_name <- name(split_cell_yymm(1))
test_time_cell <- unique_cell_centroid
test_time_cell <- test_time_cell[!test_time_cell$cell_id %in% test_timeslice$cell_id, , drop = FALSE]
write.csv(test_time_cell, "../output/bio/test_time_cell.csv", row.names = FALSE)
```
ok that works... but you need to pop into a loop and dynamically name the output (and create a .csv maybe)

need a loop to create a copy of the unique_cell_centroid dataframe, and give it the same name as the splitobs_cell_yymm level



this almost works - just need it to assign a name not a list number. note the output goes to /src....
```{r}
no_timeslices <- length(split_cell_yymm) # how many levels (time slices) are in the list
for (i in 1:no_timeslices){
  test_timeslice <- split_cell_yymm[[i]]
  test_timeslice <- as.data.frame(test_timeslice)
  test_time_cell <- unique_cell_centroid
  test_time_cell <- test_time_cell[!test_time_cell$cell_id %in% test_timeslice$cell_id, , drop = FALSE]
  write.csv(test_time_cell, paste0(i, ".csv"), row.names = FALSE)
}
