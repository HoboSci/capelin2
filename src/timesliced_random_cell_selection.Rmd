...
for (i in 1:length(bckrnd_list)){
  ...
  sel <- sample_n(sel, 20000) #where 20000 = number of rows to sample (more than 10k as some points will have NA values due to depth - I will select 10000 of these without NA values to create a large sample as per maxent)
...
backna
background_all[complete.cases(background_all), ] #removes NA values
...
```{r}
background_all <- read.csv("../output/bio/background_complete.csv")
