...
```{r}
background_all2 <- background_all[background_all$year == 2007, ]
...
write.csv(background_all_sp, "../output/bio/background_complete_2007.csv", row.names = FALSE)
...
```{r}
back1998 <- read.csv("../output/bio/background_complete_1998.csv", header = TRUE)
back1999 <- read.csv("../output/bio/background_complete_1999.csv", header = TRUE)
back2000_2003 <- read.csv("../output/bio/background_complete_2000_2003.csv", header = TRUE)
back2004_2006 <- read.csv("../output/bio/background_complete_2004_2006.csv", header = TRUE)
back2007_2011 <- read.csv("../output/bio/background_complete_2007_2011.csv", header = TRUE)
back2012_2016 <- read.csv("../output/bio/background_complete_2012_2016.csv", header = TRUE)

background_all <- rbind(back1998, back1999, back2000_2003, back2004_2006, back2007_2011, back2012_2016)
head(background_all)
```
...
```{r}
backna <- sum(!complete.cases(background_all2)) #this just counts the number of rows that contain NA values
backna
background_all2 <- background_all2[complete.cases(background_all2), ] #removes NA values
backna2 <- sum(!complete.cases(background_all)) #this just counts the number of rows that contain NA values
backna2
write.csv(background_all, "../output/bio/background_complete_na.csv", row.names = FALSE)
```

randomly select 10000 rows from each timeslice
```{r}
#background_all <- read.csv("../output/bio/background_complete_na.csv", header = TRUE)
split_cell_back <- split(background_all, list(background_all$year, background_all$month), drop = TRUE)


no_timeslices <- length(split_cell_back) # how many levels (time slices) are in the list
bckoutput <- "../output/bio/background/rand20k/no_na/" #where the files are to be saved to
for (i in 1:no_timeslices){
  timeslice <- split_cell_back[[i]]
  timeslice <- as.data.frame(timeslice)
  write.csv(timeslice, paste0(bckoutput, names(lapply(split_cell_back, names))[i], ".csv"), row.names = FALSE) 
}
```

```{r}
bckrnd_folder <- "../output/bio/background/rand20k/no_na/"      # path to folder that holds multiple .csv files
bckrnd_list <- list.files(path = bckrnd_folder, pattern="*.csv") # create list of all .csv files in folder
randbckoutput <- "../output/bio/background/rand20k/no_na_10krand/" #where the files are to be saved to
# read in each .csv file in file_list and create a data frame with the same name as the .csv file
for (i in 1:length(bckrnd_list)){
  sel <- read.csv(paste(bckrnd_folder, bckrnd_list[i], sep=''))
...
crap - the years from 2007 onwards have less than 10k points to choose from...
create a random 30k for each of these month/year periods
