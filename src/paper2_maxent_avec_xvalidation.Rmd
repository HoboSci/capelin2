...
need to create 10k background points weighted to nafo zones (as per mj)

use the exising file "../output/bio/background/correct_random_files/background_point_na_removal_nona.csv" which contains centroid location for every in the mcp....

```{r}
allbk <- read.csv("../output/bio/background/correct_random_files/background_point_na_removal_nona.csv", header = TRUE)

```

now you need to select 10k points, weighted by nafo region...
...
just checking for no background points... should be 10k per year...
```{r delete me}
prbackb <- subset(prback, occurrence == 0)
byr <- unique(prbackb$year)
```
...
for (year in 1:nrow(modelmonth)){
  ...
  newf <- paste0("../output/maxent/p2/models/", timeslice)
  dir.create(newf)
  ...
  for (h in 1:nrow(betavallist)){
    ...
    mod <- maxent(x = train, p = trainpa, args=c("maximumiterations=5000", "responsecurves=TRUE", "jackknife=TRUE", "threads=8", "addsamplestobackground=TRUE", paste0("replicates=", nrow(nfolds)), "replicatetype=crossvalidate", paste0("betamultiplier=", betamval)), path = paste0("../output/maxent/p2/models/", timeslice),"overwrite=true") #here the full path is needed
...
for (year in 1:nrow(modelmonth)){
  ...
  mod <- maxent(x = train, p = trainpa, args=c("maximumiterations=5000", "responsecurves=TRUE", "jackknife=TRUE", "threads=8", "addsamplestobackground=TRUE", paste0("replicates=", nrow(nfolds)), "replicatetype=crossvalidate", paste0("betamultiplier=", betaval)), path = paste0("../output/maxent/p2/models/", timeslice)) #here the full path is needed
