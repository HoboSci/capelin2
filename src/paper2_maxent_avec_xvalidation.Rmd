---
title: paper 2"maxent model avec xvalidation (k-fold random point test training)"
...
for (year in 1:nrow(modelmonth)){
  ...
  for (h in 1:nrow(betavallist)){
    ...
    aicect$mpa <- ecospat.mpa(samplepred$Cumulative.prediction, perc = 0.9) #90%
...
for (year in 1:nrow(modelmonth)){
  ...
  if (timeslice == "y2001" | timeslice == "y2006" | timeslice == "y2007"){
    ...
    evequal_sens_spec <- threshold(ev, 'equal_sens_spec')
      ...
      aicetc$maxkap <- evmaxkap
      aicetc$no_omission <- evno_omission
      aicetc$prevalence <- evprevalence
    ...
    #calculate minimal predicted area using ecospat
    aicetc$mpa <- ecospat.mpa(samplepred$Cumulative.prediction, perc = 0.9) #90%
    ...
    for (td in 1:nrow(unlayer_yr)){
     ...
     # pred <- predict(mod, stack, file = paste0("../output/maxent/predictions/", mthna, "/", stackn, ".asc"), overwrite=TRUE, args='outputformat=cloglog') 
      predcum <- predict(mod, stack, file = paste0("../output/maxent/predictions/", mthna, "/", stackn, ".asc"), overwrite=TRUE, args='outputformat=cumulative') 
     # prplot <- plot(pred, png(paste0("../output/maxent/predictions/", mthna, "/", stackn, ".png"))) 
      prplotcum <- plot(predcum, png(paste0("../output/maxent/predictions/", mthna, "/", stackn, ".png"))) 
