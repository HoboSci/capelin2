...
```{r}
library(ncdf4)
gebco <- nc_open("../data/env/GEBCO_2014_2D_-70.0_34.0_-43.0_70.0.nc")
gebco
library(marmap)
...
might be easier just to work form a TIF though...

First project the tiff to equal area
```{r}
library(raster)
geb <- "../data/env/gebco.tif"
...
aea
geb
geb_aea <- projectRaster(geb, aea) 
```


```{r GEBCO raster extraction}
library(rgdal)
library(sp)
gebco_coordinates(data_aea) <- c("decimalLongitude", "decimalLatitude") 
