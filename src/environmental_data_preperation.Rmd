...
note that you will still need to project the bathymetric layer to aea to do the mapping on...

I THINK THIS WILL BE SOLVED ON MY REAL PC - SAM DEAL WITH THIS!

```{r}
library(raster)
mask1 <- "../data/env/aea.tif" #just for canada pc as file isn't loaded
aea <- raster(mask1)
gebco <- raster("../data/env/GEBCO_2014_2D_-70.0_34.0_-43.0_70.0.nc")
gebco
prj <- "+proj=aea +lat_1=50 +lat_2=70 +lat_0=40 +lon_0=-60 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs"
gebco_aea <- projectRaster(gebco, prj)  
gebco_aea
```
