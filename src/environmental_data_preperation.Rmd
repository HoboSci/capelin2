...
Now subset the dataset to start in 1998 (this is when the capelin dataset for the region starts)
```{r}
amoraw <- amoraw[amoraw$year >= 1998, ]
head(amoraw)
```
...
amotimeseries <- ts(amocolumn1, frequency = 12, start = c(1998,1))
...
ok lets try a different plot, lets try see which years were on average positive (above 0) and which were negative (below 0)
...
Spring:	1 March - 31 May (col 4:6) Summer: 1 June - 31 August (col 7:9) Autumn: 1 Sept - 31 Nov (col 10:12) Winter: 1 Dec - 28 Feb (col 13, 1:2)
```{r}
amophases <- amoraw 
...
# winter has a different format - the columns are in 1, 2 & 3 which means the rowmeans function needs a bit of tweaking
amophases$WinterAvg <- rowMeans(amophases[, c(13, 2, 3)], na.rm = TRUE) #selects just cols 12, 2, & 3
```
