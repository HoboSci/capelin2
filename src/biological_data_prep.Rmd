...
packages <- c("packrat", "rworldmap", "dplyr","plyr", "rgdal", "sp", "raster", "tibble", "useful", "marmap", "weathermetrics", "fastmatch")
source("../src/ipak.R")
ipak(packages)
...
amo_yrmt
write.csv(amo_yrmt, "../output/env/amo_yrmt.csv") # written for reuse with the background points 
```
...
nao_yrmt <- nao_yrmt[order(nao_yrmt$year),] 
nao_yrmt
```
...
Note the shifted values are in a new column called amo_sample.Shifted. rename to nao_prev
...
now extract the WinterAvg.Shifted values to the species dataset #####XXXXXXX#####
...
```{r}
names(data_aea)[names(data_aea)=="freq"] <- "total_cell_obs_xy"
head(data_aea)
```
...
names(data_aea)[names(data_aea)=="freq"] <- "total_cell_obs_xyt"
head(data_aea)
```
...
```{r}
obs_cell_yymm_depth <- count(data_aea, c("cell_id", "year", "month", "depthlayerno"))
data_aea <- merge(data_aea, obs_cell_yymm_depth, by.x=c("cell_id", "year", "month", "depthlayerno"), by.y=c("cell_id", "year", "month", "depthlayerno"))
names(data_aea)[names(data_aea)=="freq"] <- "total_cell_obs_xyzt"
```
...
rename the lat/long meters columns and remove the duplicate column

```{r}
names(data_aea)[names(data_aea)=="decimalLongitude.1"] <- "longitude_meters"
names(data_aea)[names(data_aea)=="decimalLatitude.1"] <- "latitude_meters"
data_aea <- subset(data_aea, select = -c(decimalLongitude.1.1, decimalLatitude.1.1))
head(data_aea)
```
...
```{r}
data_aea$temp_celsius_depth <- kelvin.to.celsius(data_aea$temp_depth, round = 4) # 4 is the number of decimal places. the values are expressed to 4
data_aea$temp_celsius_surface <- kelvin.to.celsius(data_aea$temp_surface, round = 4)
write.csv(data_aea, "../output/bio/data_aea_cell_amo_nao_bins_env_depth_cellobno_env_naocor_cels.csv", row.names = FALSE)
```

#gear type adjustment

should not have gear not reported listed as NA (for some analysis). Change gear == NA to unknown

```{r}
data_aea$gear <- as.character(data_aea$gear)
data_aea$gear[is.na(data_aea$gea)] <- "unknown"
write.csv(data_aea, "../output/bio/data_aea_cell_amo_nao_bins_env_depth_cellobno_env_naocor_cels.csv", row.names = FALSE)
```
...
The presence points have a lot of NAs in the env. correlates. I want to: 

-ID and remove rows where all values == NA
-ID observations with no depth data that are missing surface values (will need to be removed)
-ID observations with depth data but are missing some surface and/or depth values 

how many rows(observations do you have)
```{r}
nrow(data_aea) # there are 11516 rows
```

check for no rows (observations) with NAs

```{r}
presence_noNAcount <- sum(!complete.cases(data_aea)) #this just counts the number of rows that contain NA values
presence_noNAcount
```
I have 4687 observations that contain NAs

1) find ones that have no env. data at all. these will need to be removed straight off.

There are 11516 rows in the original presence-point dataset

find the column numbers that correspond to the env. data (use fastmatch package as is quicker)

```{r}
fmatch("chl_surface", names(data_aea)) #28
fmatch("chl_depth", names(data_aea)) #29
fmatch("mlp_surface", names(data_aea)) #30
fmatch("o2_surface", names(data_aea)) #31
fmatch("o2_depth", names(data_aea)) #32
fmatch("salinity_surface", names(data_aea)) #33
fmatch("salinity_depth", names(data_aea)) #34
fmatch("ssh_surface", names(data_aea)) #35
fmatch("temp_surface", names(data_aea)) #36
fmatch("temp_depth", names(data_aea)) #37
```
ok so the env. data i am intersted in is in cols 28:37

Out of curiosoity, map the location of these points. Note the difference between this and the code below is the addition of ! on rowSums

```{r}
presence_novals <- data_aea[!rowSums(is.na(data_aea[ , 28:37])) !=10, ] #4:13 represent the columns w/ env. data, 10 is the number of columns to check (4 to 13 inclusive)
write.csv(presence_novals, "../output/bio/presence_points_without_envdata.csv", row.names = FALSE)
nrow(presence_novals) #just to check it has mapped
```

So I had 123 rows without any env. data (11516 - 11393).

plot
```{r map presences without any env. data}
map <- getMap(resolution = "low") #creates an object called map at low resoultion
plot(map, xlim = c(-70, -43), ylim =c(38,70), asp = 1) #the x and y lim are the long-lat bounds of the map
points(presence_novals$decimalLongitude, presence_novals$decimalLatitude, col = "red") #this adds points to the mapet", xlab = "Longitude", ylab = "Latitude")
```

Ok so they are along the coast, so the points probably fell outside the boxes (confirmed with closer inspection in ArcGIS).

Now remove the values
```{r}
data_aea <- data_aea[rowSums(is.na(data_aea[ , 28:37])) !=10, ] #4:13 represent the columns w/ env. data, 10 is the number of columns to check (4 to 13 inclusive)
write.csv(data_aea, "../output/bio/presence_nodata_rm")
nrow(data_aea)
```

ok so the next step is to make sure that any of the data you have without a depth value does have all relevant surface values

maybe subset

```{r}
presence_nodepth = data_aea[is.na(data_aea$depth), ]
nrow(presence_nodepth) #1215 rows
head(presence_nodepth)
```


go through each value and see which are missing data
```{r}
presence_nodepth_nochl <- presence_nodepth[is.na(presence_nodepth$chl_surface), ]
nrow(presence_nodepth_nochl) #just to check it has mapped
presence_nodepth_noo2 <- presence_nodepth[is.na(presence_nodepth$o2_surface), ]
nrow(presence_nodepth_noo2) #just to check it has mapped
presence_nodepth_nosalinity <- presence_nodepth[is.na(presence_nodepth$salinity_surface), ]
nrow(presence_nodepth_nosalinity) #just to check it has mapped
presence_nodepth_notemp <- presence_nodepth[is.na(presence_nodepth$temp_surface), ]
nrow(presence_nodepth_notemp) #just to check it has mapped
presence_nodepth_nossh <- presence_nodepth[is.na(presence_nodepth$ssh_surface), ]
nrow(presence_nodepth_nossh) #just to check it has mapped
presence_nodepth_nomlp <- presence_nodepth[is.na(presence_nodepth$mlp_surface), ]
nrow(presence_nodepth_nomlp) #just to check it has mapped
```

great! all of the values that have no depth data have surface data.

now on to the data which has depth data

```{r}
presence_wdepth = data_aea[!is.na(data_aea$depth), ]
nrow(presence_wdepth) #10178 rows
head(presence_wdepth)
```

go through each value and see which are missing data. First the surface values
```{r}
presence_wdepth_nochl <- presence_wdepth[is.na(presence_wdepth$chl_surface), ]
nrow(presence_wdepth_nochl) #just to check it has mapped
presence_wdepth_noo2 <- presence_wdepth[is.na(presence_wdepth$o2_surface), ]
nrow(presence_wdepth_noo2) #just to check it has mapped
presence_wdepth_nosalinity <- presence_wdepth[is.na(presence_wdepth$salinity_surface), ]
nrow(presence_wdepth_nosalinity) #just to check it has mapped
presence_wdepth_notemp <- presence_wdepth[is.na(presence_wdepth$temp_surface), ]
nrow(presence_wdepth_notemp) #just to check it has mapped
presence_wdepth_nossh <- presence_wdepth[is.na(presence_wdepth$ssh_surface), ]
nrow(presence_wdepth_nossh) #just to check it has mapped
presence_wdepth_nomlp <- presence_wdepth[is.na(presence_wdepth$mlp_surface), ]
nrow(presence_wdepth_nomlp) #just to check it has mapped
```

Great! All have surface values. now to the depth values

```{r}
presence_wdepth_nochl <- presence_wdepth[is.na(presence_wdepth$chl_depth), ]
nrow(presence_wdepth_nochl) #just to check it has mapped
presence_wdepth_noo2 <- presence_wdepth[is.na(presence_wdepth$o2_depth), ]
nrow(presence_wdepth_noo2) #just to check it has mapped
presence_wdepth_nosalinity <- presence_wdepth[is.na(presence_wdepth$salinity_depth), ]
nrow(presence_wdepth_nosalinity) #just to check it has mapped
presence_wdepth_notemp <- presence_wdepth[is.na(presence_wdepth$temp_depth), ]
nrow(presence_wdepth_notemp) #just to check it has mapped
```

damn... so

chl_depth = 2394 missing values
o2_depth = 2394 missing values
salinity_depth = 3109 missing values
temp_depth = 3109 missing values

This is interesting...
chl and o2 are missing the same number of points - they come from the biomer dataset; salinity and temp come from glorys and have the same number of points.

I wonder how many of these points represent the same lat-long-depth... need to subset where chl_depth OR o2_depth OR salinity_depth OR temp_depth == NA These columns are not together

```{r}
presence_wdepth2 <-  presence_wdepth[!complete.cases(presence_wdepth[28:37]), ]
nrow(presence_wdepth2) #3325
```
ok so there are 3325 points without any data.let's map these

```{r map presences with depth without any env. data at depth}
map <- getMap(resolution = "low") #creates an object called map at low resoultion
plot(map, xlim = c(-70, -43), ylim =c(38,70), asp = 1) #the x and y lim are the long-lat bounds of the map
points(presence_wdepth2$decimalLongitude, presence_wdepth2$decimalLatitude, col = "red") #this adds points to the mapet", xlab = "Longitude", ylab = "Latitude")
```
hmm everywhere...

Is there a particular time-slice that the values appear in?

```{r}
npresence_wdepth2mmyy <- with(presence_wdepth2, table(year, month))
npresence_wdepth2mmyy
```

hmm there is a lot... how does this compare to the original dataset

```{r}
data_aeammyy <- with(data_aea, table(year, month))
data_aeammyy
```

ok so 2014_07 and 1999_07 are both missing all data. Re-run the loop for this time-periods(just add the nectcdfs for these files in renetcdf folder). 

First you need to put the observations (data_aea) into a spatialpointsdataframe.

```{r}
fmatch("longitude_meters", names(data_aea)) #19
fmatch("latitude_meters", names(data_aea)) #20
```

```{r}
library(sp)
xy <- data_aea[ ,c(19,20)] # This is to tell R where the coordinates are (in column 19 and 20). Note that the column order needs to be longitude, latitude
...
```{r}
netcdf_list <- list.files("../data/env/renetcdf", pattern = '*.nc', full.names = TRUE) #true means the full path is included
no_netcdf <- length(netcdf_list) #for the loop - need to know how many files to cycle through
netcdf_name <- list.files("../data/env/renetcdf", pattern = '*.nc', full.names = FALSE) #false means the path is not includedCores
library (ncdf4)
library(sp)
library(rgdal)
...
testdata_aea <- as.data.frame(data_aea_sp)

```

lets see if that made any difference... nope ok so it wasnt that i was missing the files

now on to the data which has depth data

```{r}
presence_wdepthv2 = data_aea[!is.na(data_aea$depth), ]
nrow(presence_wdepthv2) #10178 rows - same as before
head(presence_wdepthv2)
```

```{r}
npresence_wdepth2mmyy2 <- with(presence_wdepthv2, table(year, month))
npresence_wdepth2mmyy2
```

no change...

ok is there a depth layer in particular that is an issue?
maybe some are missing a depthlayerno?

```{r}
presence_wdepth_nolayerno <- presence_wdepth2[is.na(presence_wdepth2$depthlayerno), ]
nrow(presence_wdepth_nolayerno) #0 
head(presence_wdepth_nolayerno)
```
ok so all have a depthlayerno attached

check for count of each depth layer

```{r}
whichdepths <- unique(presence_wdepth2$depthlayerno) #makes a variable of institutions
whichdepthsordered <- sort(whichdepths) #just puts the list in 
whichdepthsordered
```
quite a wide group... look at frequency

```{r count depthlayerno for data with depth but missing values}
whichdepthscount <- count(presence_wdepth2, "depthlayerno")
whichdepthscount
```
very variable...

How many rows are missing all depth data?

```{r}
presence_wdepthonly <- subset(presence_wdepth2, select = -c(mlp_surface, ssh_surface, temp_surface, salinity_surface, o2_surface, chl_surface))
head(presence_wdepthonly)
```

get the col numbers for the depth data
```{r}
fmatch("chl_depth", names(presence_wdepthonly)) #28
fmatch("o2_depth", names(presence_wdepthonly)) #29
fmatch("salinity_depth", names(presence_wdepthonly)) #30
fmatch("temp_depth", names(presence_wdepthonly)) #31
```

now subset rows where all depth data is missing
```{r}
presence_wdepthonly_allmissing <-  presence_wdepthonly[(is.na(presence_wdepthonly$temp_depth)) & (is.na(presence_wdepthonly$o2_depth)) & (is.na(presence_wdepthonly$salinity_depth)) & (is.na(presence_wdepthonly$chl_depth)), ]
nrow(presence_wdepthonly_allmissing) #2178
head(presence_wdepthonly_allmissing)
```


ok before going further, just rerun the loop again on all the data just to make sure there wasn't some silly fail...

First you need to put the observations (data_aea) into a spatialpointsdataframe.
```{r}
library(sp)
xy <- data_aea[ ,c(19,20)] # This is to tell R where the coordinates are (in column 18 and 19). Note that the column order needs to be longitude, latitude
data_aea_sp <- SpatialPointsDataFrame(coords = xy, data = data_aea, proj4string = CRS("+proj=aea +lat_1=50 +lat_2=70 +lat_0=40 +lon_0=-60 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")) # The CRS is used here is for the albers equal area projection.
```
...
write.csv(data_aea,"../output/bio/presence_points_without_envdata_relooped.csv", row.names = FALSE)
...
```{r}
presence_wdepthv3 = data_aea[!is.na(data_aea$depth), ]
nrow(presence_wdepthv3) #10178 rows - same as before whcih is good
head(presence_wdepthv3)
```

```{r}
presence_wdepth_nochl3 <- presence_wdepthv3[is.na(presence_wdepthv3$chl_depth), ]
nrow(presence_wdepth_nochl3) #just to check it has mapped
presence_wdepth_noo23 <- presence_wdepthv3[is.na(presence_wdepthv3$o2_depth), ]
nrow(presence_wdepth_noo23) #just to check it has mapped
presence_wdepth_nosalinity3 <- presence_wdepthv3[is.na(presence_wdepthv3$salinity_depth), ]
nrow(presence_wdepth_nosalinity3) #just to check it has mapped
presence_wdepth_notemp3 <- presence_wdepthv3[is.na(presence_wdepthv3$temp_depth), ]
nrow(presence_wdepth_notemp3) #just to check it has mapped
```
...
ok well i think the answer here is that if i am going to use depth data, i should get rid of those with missing data. if i am going to use surface only data, then i can keep all points.



# glorys bathymetric data

so i just found that glorys also has a bathymetric data layer...
...
then covert the netcdf with the bathy layer (GLO-MFC_001_025_bathy.nclonlat.nc) to a brick and project to the albers equal area
```{r}
bathyrast <- raster("../data/env/GLO-MFC_001_025_bathy.nclonlat.nc") #because it is a single level file
aea <- raster("../output/env/aea.tif") 
bathyrast <- projectRaster(bathyrast, aea)
plot(bathyrast)
```

now extract and add to data_aea
```{r}
data_aea_sp$bottom_depth_glorys <- extract(x = bathyrast, y = data_aea_sp)
data_aea <- as.data.frame(data_aea_sp) # this coverts the 'spatialpoints dataframe' to a 'normal dataframe'
write.csv(data_aea, file = "../output/bio/presence_points_without_envdata_relooped_glbathy.csv", row.names = FALSE) # change the path to where ever you would like to save it
head(data_aea)
```

# remove any duplicate entries

Check to see how many cells have >1 point per timeslice. This needs to be timesliced and 3D

```{r cell_id_3d}
data_aea$cell_id_3d <- paste(data_aea$cell_id, data_aea$depthlayerno, sep="_") #create a new column that is a unique cell_id & depth ID. 
head(data_aea)
```

and now do one with a timeslice label
```{r cell_id_xyzt}
data_aea$cell_id_xyzt <- paste(data_aea$cell_id_3d, data_aea$year, data_aea$month, sep="_") #create a new column that is a unique cell_id & depth ID. 
head(data_aea)
write.csv(data_aea, "../output/bio/presence_points_without_envdata_relooped_glbathy_cellid_xyzt.csv", row.names = FALSE)
```

ok lets see how many have more than one obs per cell
```{r unique_pointpercell}
unique_pointpercell <- unique(data_aea$total_cell_obs_xyzt)
unique_pointpercell
```

bugger... ok so lets do a frequency table
```{r unique_pointpercell frequencies}
celobsmore1 <- subset(data_aea, total_cell_obs_xyzt >1)
celobsmore1 #2042 rows
...
write.csv(pointpercellcount, file = "../output/bio/pointpercellcount_pres.csv")
pointpercellcount
```
...
```{r}
fmatch("cell_id_xyzt", names(celobsmore1)) #50
duplicatesbegone <- celobsmore1[!duplicated(celobsmore1[ ,50]), ] #creates a df with all duplicates gone (now 1 obs per cell)
nrow(duplicatesbegone) #949 - good
presabnodup <- subset(data_aea, total_cell_obs_xyzt == "1") #remove all rows that have more than one obs
nrow(presabnodup) #9351 - this adds up 
data_aea <- rbind(presabnodup, duplicatesbegone)
nrow(data_aea) #10300 - great!
```
...
check for unique values(should be 1 only)
```{r}
unique_pointpercell <- unique(data_aea$total_cell_obs_xyzt)
unique_pointpercell
```
...
```{r}
write.csv(data_aea, file = "../output/bio/presence_points_without_envdata_relooped_glbathy_nodup.csv", row.names = FALSE) # change the path to where ever you would like to save it
```
