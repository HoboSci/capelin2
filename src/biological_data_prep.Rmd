...
packages <- c("packrat", "rworldmap", "dplyr","plyr", "rgdal", "sp", "raster", "tibble", "useful")
source("../src/ipak.R")
ipak(packages)
...
#add columns for the env. variables
The ones ypu have to add are...
...
populate the columns with NA
```{r}
data_aea$mlp <- NA
data_aea$salinty_1 <- NA
data_aea$salinity_depth <- NA
data_aea$ssh <- NA
data_aea$temp_1 <- NA
data_aea$temp_depth <- NA
data_aea$chl_1 <- NA
data_aea$chl_depth <- NA
data_aea$o2_1 <- NA
data_aea$o2_depth <- NA
write.csv(data_aea,"../output/bio/data_aea_cell_amo_nao_bins_env.csv", row.names = FALSE)
```
