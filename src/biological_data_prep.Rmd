...
salinity_depth = 3133 missing values
temp_depth = 3109 missing values

This is interesting...
chl and o2 are missing the same number of points - they come from the biomer dataset; salinity and temp come from glorys and have different values.
...
nrow(presence_wdepth2) #3349
```
ok so there are 3349 points without any data.let's map these
...
ok so 2014_07 and 1999_07 are both missing all data. Re-run the loop for these two time-periods (just add the nectcdfs for these files in renetcdf folder). 
...
lets see if that made any difference...
...
so there are 2178 out of 3349 observations that are missing all data for depth. so what about the remaining 1171 points?

want to remove from the observations in presence_wdepthonly (all 3349 observations) the ones that appear in presence_wdepthonly_allmissing (2178 observations)

some code from [stack exchange](https://stackoverflow.com/questions/10907359/r-remove-rows-from-one-data-frame-that-are-in-another)

```{r}
presence_wdepthonly_somemissing <-  rbind(presence_wdepthonly_allmissing, presence_wdepthonly)
presence_wdepthonly_somemissing <- presence_wdepthonly_somemissing[!duplicated(presence_wdepthonly_somemissing, fromLast = FALSE) & !duplicated(presence_wdepthonly_somemissing, fromLast = TRUE), ]
nrow(presence_wdepthonly_somemissing)
head(presence_wdepthonly_somemissing)
```

ok before going further, just rerun the loop again on all the data...
...
[1] 3133
[1] 3109
so actually a minor change (maybe this is because you added the dec 2015 data in - it wasn't available before). ok need to redo checks...
...
ok great - now let's do the same thing for the bottom_layer_no (yes Glorys has a layer for this as well!) 

 covert the netcdf with the bottom layer nor (GLO-GLO-MFC_001_025_bottomlayerno.nclonlat.nc) to a raster and project to the albers equal area
```{r}
botlayernorast <- raster("../data/env/GLO-MFC_001_025_bottomlayerno.nclonlat.nc") #because it is a single level file
botlayernorast <- projectRaster(botlayernorast, aea)
plot(botlayernorast)
```

now extract and add to data_aea_sp, but this time also change the sp to a normal dataframe and save as a .csv
```{r}
data_aea_sp$bottom_layerno_glorys <- extract(x = botlayernorast, y = data_aea_sp)
write.csv(data_aea_sp, file = "presence_points_without_envdata_relooped_glbathy.csv", row.names = FALSE) # change the path to where ever you would like to save it
data_aea <- as.data.frame(data_aea_sp) # this coverts the 'spatialpoints dataframe' to a 'normal dataframe'
head(data_aea)
```

ok bottom layer no is not right