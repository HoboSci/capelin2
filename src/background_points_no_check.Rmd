...
xy <- testbk100000[ ,c("longitude_","latitude_m")] # This is to tell R where the coordinates are. Note that the column order needs to be longitude, latitude
testbk100000sp <- SpatialPointsDataFrame(coords = xy, data = testbk100000, proj4string = CRS("+proj=aea +lat_1=50 +lat_2=70 +lat_0=40 +lon_0=-60 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")) # The CRS is used here is for the albers equal area projection.
...
for (i in 1:no_netcdf) {  
  ...
  temp_brick <- projectRaster(temp_brick, aea) 
    for (j in 1:nrow(testbk100000sp)) {  
      de <- testbk100000sp$depthlayerno[[j]]  # a variable for the observation depth layer
          if (brkyr == yr & brkmth == mth & brkvar == "temp.nc"){
              testbk100000sp$temp_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ]) 
              if (is.na(de)){
                testbk100000sp$temp_depth[j] <- NA
              } else  
                testbk100000sp$temp_depth[j] <- extract(x=temp_brick[[de]], y = testbk100000sp[j, ])
          } else if (brkyr == yr & brkmth == mth & brkvar == "salinity.nc") {
              testbk100000sp$salinity_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ]) 
              if (is.na(de)){
                testbk100000sp$salinity_depth[j] <- NA
              } else  
                testbk100000sp$salinity_depth[j] <- extract(x=temp_brick[[de]], y = testbk100000sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "chl.nc") {
              testbk100000sp$chl_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ]) 
              if (is.na(de)){
                testbk100000sp$chl_depth[j] <- NA
              } else  
                testbk100000sp$chl_depth[j] <- extract(x=temp_brick[[de]], y = testbk100000sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "o2.nc") {
              testbk100000sp$o2_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ]) 
              if (is.na(de)){
                testbk100000sp$o2_depth[j] <- NA
              } else  
                testbk100000sp$o2_depth[j] <- extract(x=temp_brick[[de]], y = testbk100000sp[j, ]) 
          } else if (brkyr == yr & brkmth == mth & brkvar == "mlp.nc") {
              testbk100000sp$mlp_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ])
          } else if (brkyr == yr & brkmth == mth & brkvar == "ssh.nc") {
              testbk100000sp$ssh_surface[j] <- extract(x=temp_brick[[1]], y = testbk100000sp[j, ]) 
            
          }
     
    }
}
write.csv(testbk100000sp, "../data/testbk100000output.csv", row.names = FALSE)
...
ggplot(test10000, aes(x = ssh_surface)) + geom_density(na.rm = TRUE, colour = "red") + geom_density(data=test20000 , na.rm = TRUE, colour = "blue") + geom_density(data=test50000 , na.rm = TRUE, colour = "green")
...
ggplot(test10000, aes(x = mlp_surface)) + geom_density(na.rm = TRUE, colour = "red") + geom_density(data=test20000 , na.rm = TRUE, colour = "blue") + geom_density(data=test50000 , na.rm = TRUE, colour = "green")
