...
packages <- c("rJava", "dismo", "stringr", "raster", "ecospat", "plyr", "data.table", "RColorBrewer")
source("../src/ipak.R")
ipak(packages)
```
...
mod <- maxent(x = train, p = trainpa, path = paste0("../output/maxent/all_data/models"), args=c("maximumiterations=5000", "threads=8", "addsamplestobackground=TRUE", paste0("replicates=", nrow(nfolds)), "replicatetype=crossvalidate", paste0("betamultiplier=", betaval)))
...
unlayer_yr <- subset(unlayer_yrX, ascmonths == "7")
...
temp_depth <- raster(unlayer_yr$temp_depth[td])
      ...
      pred <- predict(mod, stack, file = paste0("../output/maxent/all_data/predictions/", "cloglog_", stackn, ".asc"), overwrite=TRUE, args='outputformat=cloglog') 
...
asc_list <- list.files(paste0("../output/maxent/all_data/predictions/"), pattern = "cloglog.*.asc", full.names = TRUE)
...
write.csv(depthlayers, ("../output/maxent/all_data/predictions/cloglog_asclayerbydepth.csv"), row.names = FALSE)
...
for (x in 1:nrow(depthlayers)){
  ...
  for (ly in 1:length(asclay)){
    ...
    if(ly == length(asclay)){
      ...
      writeRaster(stavg, (paste0("../output/maxent/all_data/predictions/avg_", dep, ".asc")), overwrite=TRUE)
      avgplot <- plot(stavg, png(paste0("../output/maxent/all_data/predictions/avg_", dep, ".png")))
