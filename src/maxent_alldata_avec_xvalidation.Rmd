...
packages <- c("rJava", "dismo", "stringr", "raster", "ecospat")
source("../src/ipak.R")
ipak(packages)
```
...
```{r}
allvar <- c("occurrence", "year", "temp_surface", "temp_depth", "salinity_surface", "salinity_depth", "o2_depth", "chl_surface", "mlp_surface",  "nao_sample", "nao_prev", "nao_winter", "amo_sample", "amo_winter")
presback <- subset(presback, select = c(allvar))
colnames(presback)
```
...
nfolds <- cbind(c(1:5))
  ...
  for (h in 1:nrow(betavallist)){
    ...
    mod <- maxent(x = train, p = trainpa, path = paste0("../output/maxent/", timeslice, "/models"), args=c("maximumiterations=5000", "responsecurves=TRUE", "jackknife=TRUE", "threads=8", "addsamplestobackground=TRUE", paste0("replicates=", nrow(nfolds)), "replicatetype=crossvalidate", paste0("betamultiplier=", betamval))) 
...
for(f in 1:nrow(nfolds)){
  modn <- nfolds[f]-1
    ...
    aicetc$AIC <- ((2*aicetc$coefficients)-(2*aicetc$sum.of.logs))
    aicetc$AICc <- (-2*(aicetc$sum.of.logs)+((2*aicetc$coefficients)*(aicetc$n/(aicetc$n - aicetc$coefficients-1))))
    write.csv(aicetc, paste0("../output/maxent/", timeslice, "/models/", betamval, "_", modn, "_", "AICforMaxentReg.csv"), row.names = FALSE)
      }
    
  # aa <- paste0("fold", modn, "_", betamval, "_", "AICforMaxentReg.csv")
    aicperbetalst <- list.files(paste0("../output/maxent/", timeslice, "/models/"), pattern = paste0("^", betamval, ".*AICforMaxentReg.csv"), full.names = TRUE)
    ...
    write.csv(aicperbeta, paste0("../output/maxent/", timeslice, "/models/aicperbetasum_", betamval, ".csv"), row.names = FALSE)
    
    betaavglst <- list.files(paste0("../output/maxent/", timeslice, "/models/"), pattern = "aicperbetasum_.*csv", full.names = TRUE)
 ...
 # timeslice <- modelmonth[year] #testing timeslice
  ...
  if (yr == "2003" | yr == "2004"){
   ...
   for (td in 1:nrow(unlayer_yr)){
      ...
      pred <- predict(mod, stack, file = paste0("../output/maxent/predictions/", mthna, "/", "cloglog_", stackn, ".asc"), overwrite=TRUE, args='outputformat=cloglog') 
      predcum <- predict(mod, stack, file = paste0("../output/maxent/predictions/", mthna, "/", "cumu_", stackn, ".asc"), overwrite=TRUE, args=c('outputformat=Cumulative')) 
...
asc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "cloglog*.asc", full.names = TRUE)
...
clog_asc120  <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_120.asc"))
clog_asc41.18 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_41.18.asc"))
clog_asc1.55586 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_1.55586.asc"))
clog_asc1045.85 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_1045.85.asc"))
clog_asc108.03 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_108.03.asc"))
clog_asc11.7737 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_11.7737.asc"))
clog_asc1151.99 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_1151.99.asc"))
clog_asc1265.86 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_1265.86.asc"))
clog_asc13.991 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_13.991.asc"))
clog_asc133.076 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_133.076.asc"))
clog_asc1387.38 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_1387.38.asc"))
clog_asc147.406 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_147.406.asc"))
clog_asc16.5253 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_16.5253.asc"))
clog_asc163.165 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_163.165.asc"))
clog_asc180.55 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_180.55.asc"))
clog_asc19.4298 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_19.4298.asc"))
clog_asc199.79 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_199.79.asc"))
clog_asc2.66768 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_2.66768.asc"))
clog_asc22.7576 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_22.7576.asc"))
clog_asc221.141 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_221.141.asc"))
clog_asc244.891 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_244.891.asc"))
clog_asc26.5583 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_26.5583.asc"))
clog_asc271.356 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_271.356.asc"))
clog_asc3.85628 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_3.85628.asc"))
clog_asc30.8746 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_30.8746.asc"))
clog_asc300.888 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_300.888.asc"))
clog_asc333.863 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_333.863.asc"))
clog_asc35.7402 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_35.7402.asc"))
clog_asc370.689 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_370.689.asc"))
clog_asc411.794 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_411.794.asc"))
clog_asc457.626 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_457.626.asc"))
clog_asc47.2119 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_47.2119.asc"))
clog_asc5.14036 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_5.14036.asc"))
clog_asc508.64 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_508.64.asc"))
clog_asc53.8506 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_53.8506.asc"))
clog_asc565.292 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_565.292.asc"))
clog_asc6.54303 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_6.54303.asc"))
clog_asc61.1128 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_61.1128.asc"))
clog_asc628.026 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_628.026.asc"))
clog_asc69.0217 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_69.0217.asc"))
clog_asc697.259 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_697.259.asc"))
clog_asc77.6112 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_77.6112.asc"))
clog_asc773.368 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_773.368.asc"))
clog_asc8.09252 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_8.09252.asc"))
clog_asc856.679 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_856.679.asc"))
clog_asc86.9294 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_86.9294.asc"))
clog_asc9.82275 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_9.82275.asc"))
clog_asc947.448 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_947.448.asc"))
clog_asc97.0413 <- raster(paste0("../output/maxent/predictions/", mthna, "/avg_97.0413.asc"))
...
stsum <- sum(stackavg, na.rm = TRUE)
  writeRaster(stsum, (paste0("../output/maxent/predictions/", mthna, "/clog_sum_", mthna, ".asc")), overwrite=TRUE)
  sumplot <- plot(stsum, png(paste0("../output/maxent/predictions/", mthna, "/clog_sum_", mthna, ".png"))) 
  dev.off()
stavg <- calc(stackavg, fun = mean, na.rm = TRUE)
  writeRaster(stavg, (paste0("../output/maxent/predictions/", mthna, "/clog_avg_", mthna, ".asc")), overwrite=TRUE)
  avgplot <- plot(stavg, png(paste0("../output/maxent/predictions/", mthna, "/clog_avg_", mthna, ".png"))) 
  dev.off()
...
there seem to be two primary ways to create binary maps. the most common for conservation purposes seems to be to use maxsumspensspec values, however MPA also comes recommended (but is def less common). probably best to use cumulative for both (merrow 2013)
...
thresholds <- tail(thresholds, 1)
mpa <- thresholds$mpa_0.9_cum
maxsesp <- thresholds$test_maxsumsenspec
cumasc_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^cumu_.*.asc", full.names = TRUE) 
depthlayers <- data.frame(cbind(unique(stdep$ascdepths)))
names(depthlayers)[names(depthlayers) == "cbind.unique.stdep.ascdepths.."] <- "depth"
depthlayers$depth <- as.character(depthlayers$depth)
...
write.csv(depthlayers, (paste0("../output/maxent/predictions/", mthna, "/cumu_asclayerbydepth.csv")), row.names = TRUE)
...
for (x in 1:nrow(depthlayers)){
  ...
  writeRaster(avg, (paste0("../output/maxent/predictions/", mthna, "/cumu_avg_", asclaydep, ".asc")), overwrite=TRUE, args='outputformat=cumulative')

}

cumavg_list <- list.files(paste0("../output/maxent/predictions/", mthna), pattern = "^cumu_avg_.*.asc", full.names = TRUE) 
for(lyr in 1:length(cumavg_list)){
  avgras <- raster(cumavg_list[lyr])
  reclassmpa <- c(mpa, Inf, 1)
  reclassmpa <- matrix(reclassmpa, ncol = 3, byrow = TRUE)
  binavgras <- reclassify(avgras, reclassmpa)
}
