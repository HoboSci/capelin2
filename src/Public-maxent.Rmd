---
title: "maxent model plus plot creation"
...
options(java.parameters = "-Xmx3g")#needs to run before dismo loaded - boosts memory for java to 3g
packages <- c("dismo", "stringr", "rJava", "raster", "ecospat", "bossMaps", "lattice", "enmSdm", "ggplot2", "plyr", "reshape2", "tidyverse", "patchwork", "purrr", "Hmisc", "cowplot", "sf", "lubridate")
source("../src/ipak.R")
ipak(packages)
```
...
presback <- read.csv("../output/bio/presab_nafo_yr.csv", header = TRUE) # this file is available in the Figshare project
```

For each monthly model, subset the data to the month you want, and the variables you want (Based on spearmans and VIF)
Ex: edit this list for variables used in each month (or simply replace with a single list if, as in this case, months have the same variables)
...
for (year in 1:nrow(modelmonth)){
  ...
  for (h in 1:nrow(betavallist)){
   ...
   #test auc scores and other lovely things you might want to include e.g.
...
AICc selection
select the models you want for each year and run those models again (primarily based on lowest AICc but check test AUC and other things too)
...
did this after splitting layers back into their individual depths as per Bentlage 2013 https://doi.org/10.1111/geb.12049
...
#the following cde was manually edited for each month
...
perm2 <- melt(permu, id = "Month") #need to melt the data into this format for ggplot
perm2$variable <- factor(perm2$variable, levels = c("AMO_Sample", "AMO_Winter", "NAO_Sample", "NAO_Prev", "NAO_Winter", "Chl_Surface", "O2_Depth", "Salinity_Depth", "Temp_Depth")) #reorder the variables
perpl <- ggplot(data = perm2, aes(x = Month, y = value, fill = variable)) + geom_bar(stat = "identity") +  scale_fill_viridis_d(direction = 1, labels = c("AMO Sample", "AMO Winter", "NAO Sample", "NAO Prev", "NAO Winter", "Chl Surface", "O2 Depth", "Salinity Depth", "Temp Depth")) + theme_half_open(12) + scale_x_discrete(limits = mthna) + labs(y = "Permutation Importance (%)")
...
ggsave("../output/maxent/paper_plots/prob-overview.png", width = 210, height = 297, units = "mm", dpi = 600)
```
...
#stats reported in the manuscript
