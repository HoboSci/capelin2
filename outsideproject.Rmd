...
packages <- c("ncdf4", "raster", "foreach", "doParallel", "doMC") 
source("C:/Users/bunny/Documents/GitHub/Chapter_1/src/ipak.R")
ipak(packages)
```
...
```{r}
test_back_or <- read.csv("C:/Users/bunny/Documents/GitHub/Chapter_1/data/looptest.csv", header = TRUE) #test dataset
...
netcdf_list <- list.files("C:/Users/bunny/Documents/GitHub/Chapter_1/data/testloopnetcdf", pattern = '*.nc', full.names = TRUE) #true means the full path is included
no_netcdf <- length(netcdf_list) #for the loop - need to know how many files to cycle through
netcdf_name <- list.files("C:/Users/bunny/Documents/GitHub/Chapter_1/data/testloopnetcdf", pattern = '*.nc', full.names = FALSE) #false means the path is not included
aea <- raster("C:/Users/bunny/Documents/GitHub/Chapter_1/output/env/aea.tif") 
...
write.csv(test_back, "C:/Users/bunny/Documents/GitHub/Chapter_1/data/testloopoutput.csv", row.names = FALSE)
...
netcdf_name <- list.files("C:/Users/bunny/Documents/GitHub/Chapter_1/data/testloopnetcdf", pattern = '*.nc', full.names = FALSE) #false means the path is not included
aea <- raster("C:/Users/bunny/Documents/GitHub/Chapter_1/output/env/aea.tif") 
foreach ((i = 1:no_netcdf) , .packages = c("raster", "ncdf4")) %do% {  
  print(netcdf_name[i]) #this just prints the name of the netCDF R is working one
  brkyr <- as.integer(sapply(strsplit(netcdf_name[i], "_"), "[[", 1)) # extracting the first part of the netcdf filename (which is the year)
  brkmth <- as.integer(sapply(strsplit(netcdf_name[i], "_"), "[[", 2)) # extracting the second part of the netcdf filename (which is the month)
  brkvar <- (sapply(strsplit(netcdf_name[i], "_"), "[[", 3)) # extracting the third part of the netcdf (inc.nc)
